...
#undef CLEAN
	...
	if (frandom) fclose(frandom);
		...
		srv->is_real_entropy = 0;
		for (j = 0; j < sizeof(srv->entropy); j++)
			sgx_read_rand((unsigned char*)&srv->entropy[j], sizeof(srv->entropy[j]));
#endif
...
{
	my_fprintf(stderr, "Applying listen_backlog: %d\n", srv->srvconf.listen_backlog);
		   ...
		   server *first_entry, const char *conf_file)
{
	...
	*srv_states = (server **)calloc(cpus, sizeof(server *));
	if (NULL == *srv_states) {
		my_fprintf(stderr, "%s: %d(%s) - Can't allocate memory for srv_states\n",
	...
	for (i = 1; i < cpus; i++) {
		...
		if (NULL == ((*srv_states)[i] = server_init())) {
			my_fprintf(stderr, "%s: %d(%s) - Can't allocate memory for %dth srv_state entry\n", 
...
start_server(void *svrptr)
{
	...
	void* ret = NULL;
#if defined(USE_MTCP) && defined(ENABLE_UCTX)
	mtcp_create_app_context(srv->mctx, (mtcp_app_func_t) MainLoop, (void *) svrptr);
	mtcp_run_app(srv->mctx);
#else
	ret = MainLoop(svrptr);
#endif
	return ret;
...
#if 0
					if (cs == 0) {
						my_fprintf(stderr, "connection-state: ");
						cs = 1;
					}

					my_fprintf(stderr, "c[%d,%d]: %s ",
						con->fd,
						con->fcgi.fd,
...
main_wrapper(int argc, char **argv) {
	...
	if (NULL == (srv = server_init())) {
		my_fprintf(stderr, "did this really happen?\n");
		return EXIT_FAILURE;
	}
	...
	while(-1 != (o = getopt(argc, argv, "f:m:n:hvVDpt"))) {
		...
		case 'f':
			...
			conf_file = strndup(optarg, 4096);
			if (NULL == conf_file) {
				my_fprintf(stderr, "Can't duplicate conf_file string\n");
				return EXIT_FAILURE;
			}
#endif
...
#else
			my_fprintf(stderr, "-n option only works with MTCP/MULTI_THREADED support!\n");
			exit(EXIT_FAILURE);
#endif
			break;
	...
	if (print_config) {
		...
		if (dc) {
			...
			/* shouldn't happend */
			my_fprintf(stderr, "global config not found\n");
		}
	}
	...
	if (srv->srvconf.pid_file->used) {
		if (-1 == (pid_fd = open(srv->srvconf.pid_file->ptr, O_WRONLY | O_CREAT | O_EXCL | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH))) {
			...
			if (0 != wrapper_stat(srv->srvconf.pid_file->ptr, &st)) {
				log_error_write(srv, __FILE__, __LINE__, "sbs",
	...
	if (i_am_root) {
...
#ifdef HAVE_GETRLIMIT
		if (0 != wrapper_getrlimit(RLIMIT_NOFILE, &rlim)) {
			log_error_write(srv, __FILE__, __LINE__,
		...
		if (use_rlimit && srv->srvconf.max_fds) {
			...
			if (0 != wrapper_setrlimit(RLIMIT_NOFILE, &rlim)) {
				log_error_write(srv, __FILE__, __LINE__,
		...
		if (use_rlimit && srv->srvconf.enable_cores && wrapper_getrlimit(RLIMIT_CORE, &rlim) == 0) {
			rlim.rlim_cur = rlim.rlim_max;
			wrapper_setrlimit(RLIMIT_CORE, &rlim);
		}
#endif
		if (srv->event_handler == FDEVENT_HANDLER_SELECT) {
		...
		/* set core file rlimit, if enable_cores is set */
		if (srv->srvconf.enable_cores && wrapper_getrlimit(RLIMIT_CORE, &rlim) == 0) {
			rlim.rlim_cur = rlim.rlim_max;
			wrapper_setrlimit(RLIMIT_CORE, &rlim);
		}
...
#endif
		if (thread_create(&srv_states[i]->running_thread, NULL,
				   start_server, (void *)srv_states[i])) {
		  goto clean_up;
		}
		sleep(1);
...
# endif
...
#ifdef COMPILE_WITH_INTEL_SGX
	// for some reason, with SGX, the main thread just wants to exit
	while (1) {
		sleep(1);
	}
#endif
	...
	/* main thread waits... */
	for (i = 0; i < cpus; i++)
		thread_join(srv_states[i]->running_thread, NULL);
 clean_up:
#ifdef USE_MTCP
