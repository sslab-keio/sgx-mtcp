...
network_server_init(server *srv, buffer *host_token, specific_config *s) {
	...
	if (NULL == srv_socket) {
		my_fprintf(stderr, "%s: %d(%s) - Can't allocate memory for srv_socket\n",
	...
	*(sp++) = '\0';
	...
	case AF_UNIX:
		...
		strncpy(srv_socket->addr.un.sun_path, host, 108);
	...
	if (s->is_ssl) {
		...
		strncpy(afa.af_name, "httpready", 10);
	...
	//#ifndef USE_MTCP
		...
		while (srv->first_entry->srv_sockets.used == 0) { my_fprintf(stdout, " "); }
		...
		if (srv->srv_sockets.size == 0) {
			/* my_fprintf(stderr, "%u: 1 Do i come here", srv->cpu); */
			srv->srv_sockets.size = 4;
			srv->srv_sockets.used = 0;
			srv->srv_sockets.ptr = malloc(srv->srv_sockets.size * sizeof(server_socket));
		} else if (srv->srv_sockets.used == srv->srv_sockets.size) {
			/* my_fprintf(stderr, "%u: 2 Do i come here", srv->cpu); */
			srv->srv_sockets.size += 4;
			srv->srv_sockets.ptr = realloc(srv->srv_sockets.ptr, srv->srv_sockets.size * sizeof(server_socket));
		}
