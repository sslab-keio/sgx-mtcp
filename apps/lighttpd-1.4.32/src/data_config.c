...
static void data_config_print(const data_unset *d, int depth) {
	...
	if (0 == ds->context_ndx) {
		my_fprintf(stdout, "config {\n");
	}
	else {
		my_fprintf(stdout, "$%s %s \"%s\" {\n",
				ds->comp_key->ptr, ds->op->ptr, ds->string->ptr);
		array_print_indent(depth + 1);
		my_fprintf(stdout, "# block %d\n", ds->context_ndx);
	}
	depth ++;
	...
	for (i = 0; i < a->used; i ++) {
		...
		my_fprintf(stdout, "%s", du->key->ptr);
	...
	my_fprintf(stdout, "}");
	if (0 != ds->context_ndx) {
		my_fprintf(stdout, " # end of $%s %s \"%s\"",
				ds->comp_key->ptr, ds->op->ptr, ds->string->ptr);
	}
