...
int config_read(server *srv, const char *fn) {
	...
	if (NULL != (modules = (data_array *)array_get_element(srv->config, "server.modules"))) {
		...
		if (modules->type != TYPE_ARRAY) {
			my_fprintf(stderr, "server.modules must be an array");
			return -1;
		}
...
int config_set_defaults(server *srv) {
	...
	if (buffer_is_empty(srv->srvconf.changeroot)) {
		if (-1 == wrapper_stat(s->document_root->ptr, &st1)) {
			log_error_write(srv, __FILE__, __LINE__, "sb",
		...
		if (-1 == wrapper_stat(srv->tmp_buf->ptr, &st1)) {
			log_error_write(srv, __FILE__, __LINE__, "sb",
					"base-docroot doesn't exist:",
					srv->tmp_buf);
			return -1;
	...
	if (2 == s->force_lowercase_filenames) { /* user didn't configure it in global section? */
		...
		if (0 == wrapper_stat(srv->tmp_buf->ptr, &st1)) {
			int is_lower = 0;
			...
			} else if (0 == wrapper_stat(srv->tmp_buf->ptr, &st2)) {
