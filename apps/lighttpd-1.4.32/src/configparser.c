...
static void configparser_push(config_t *ctx, data_config *dc, int isnew) {
  ...
  if (ctx->configs_stack->used > 0 && ctx->current->context_ndx == 0) {
    my_fprintf(stderr, "Cannot use conditionals inside a global { ... } block\n");
    exit(-1);
  }
...
#if 0
  my_fprintf(stderr, "get var %s\n", key->ptr);
#endif
  for (dc = ctx->current; dc; dc = dc->parent) {
#if 0
    my_fprintf(stderr, "get var on block: %s\n", dc->key->ptr);
...
data_unset *configparser_merge_data(data_unset *op1, const data_unset *op2) {
  ...
  if (op1->type != op2->type) {
    if (op1->type == TYPE_STRING && op2->type == TYPE_INTEGER) {
      ...
      my_fprintf(stderr, "data type mismatch, cannot merge\n");
      return NULL;
    }
  }
...
#line 146 "./configparser.y"
{
  if (ctx->ok) {
    ...
    if (strncmp(yymsp[-2].minor.yy43->ptr, "env.", sizeof("env.") - 1) == 0) {
      my_fprintf(stderr, "Setting env variable is not supported in conditional %d %s: %s\n",
      ...
      my_fprintf(stderr, "Duplicate config variable in conditional %d %s: %s\n",
              ctx->current->context_ndx,
...
#line 170 "./configparser.y"
{
  ...
  if (strncmp(yymsp[-2].minor.yy43->ptr, "env.", sizeof("env.") - 1) == 0) {
    my_fprintf(stderr, "Appending env variable is not supported in conditional %d %s: %s\n",
        ctx->current->context_ndx,
        ctx->current->key->ptr, yymsp[-2].minor.yy43->ptr);
    ctx->ok = 0;
...
#line 290 "./configparser.y"
{
  if (buffer_is_empty(yymsp[0].minor.yy41->key) ||
    ...
    my_fprintf(stderr, "Duplicate array-key: %s\n",
...
#line 352 "./configparser.y"
{
  if (yymsp[-3].minor.yy78->context_ndx >= yymsp[0].minor.yy78->context_ndx) {
    my_fprintf(stderr, "unreachable else condition\n");
    ctx->ok = 0;
  }
...
#line 380 "./configparser.y"
{
  ...
  if (ctx->ok && yymsp[0].minor.yy41->type != TYPE_STRING) {
    my_fprintf(stderr, "rvalue must be string");
    ctx->ok = 0;
  }
  ...
  if (NULL != (dc = (data_config *)array_get_element(ctx->all_configs, b->ptr))) {
    ...
    if (COMP_UNSET == dc->comp) {
      my_fprintf(stderr, "error comp_key %s", dc->comp_key->ptr);
      ctx->ok = 0;
    }
    ...
    case CONFIG_COND_MATCH: {
      ...
      if (NULL == (dc->regex =
        ...
        my_fprintf(stderr, "parsing regex failed: %s -> %s at offset %d\n",
        ...
        ctx->ok = 0;
          ...
          pcre_study(dc->regex, 0, &errptr)) &&
                 errptr != NULL) {
        my_fprintf(stderr, "studying regex failed: %s -> %s\n",
            rvalue->ptr, errptr);
        ctx->ok = 0;
      } else if (0 != (pcre_fullinfo(dc->regex, dc->regex_study, PCRE_INFO_CAPTURECOUNT, &captures))) {
        my_fprintf(stderr, "getting capture count for regex failed: %s\n",
            rvalue->ptr);
        ctx->ok = 0;
      } else if (captures > 9) {
        my_fprintf(stderr, "Too many captures in regex, use (?:...) instead of (...): %s\n",
            rvalue->ptr);
        ctx->ok = 0;
      } else {
        dc->string = buffer_init_buffer(rvalue);
...
#else
      my_fprintf(stderr, "can't handle '$%s[%s] =~ ...' as you compiled without pcre support. \n"
    ...
    default:
      my_fprintf(stderr, "unknown condition for $%s[%s]\n",
                      yymsp[-5].minor.yy0->ptr, yymsp[-3].minor.yy43->ptr);
      ctx->ok = 0;
      break;
    }
...
#line 542 "./configparser.y"
{
  ...
  if (ctx->ok) {
    if (yymsp[0].minor.yy41->type == TYPE_STRING) {
      ...
      my_fprintf(stderr, "operand must be string");
      ctx->ok = 0;
    }
  }
  yymsp[0].minor.yy41->free(yymsp[0].minor.yy41);
