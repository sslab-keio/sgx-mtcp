...
#else
#define wrapper_mmap(addr, length, prot, flags, fd, offset) mmap(addr, length, prot, flags, fd, offset)
...
int stream_open(stream *f, buffer *fn) {
	...
	f->start = wrapper_mmap(NULL, f->size, PROT_READ, MAP_SHARED, fd, 0);
