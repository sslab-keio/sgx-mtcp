...
#define wrapper_syslog(priority, format, ...) syslog(priority, format, ##__VA_ARGS__)
#endif
...
int open_logfile_or_pipe(server *srv, const char* logfile) {
	...
	if (logfile[0] == '|') {
		my_fprintf(stderr, "I am creating a pipe here!\n");
#ifdef HAVE_FORK
...
int log_error_write(server *srv, const char *filename, unsigned int line, const char *fmt, ...) {
	...
	case ERRORLOG_FD:
		...
		break;
	case ERRORLOG_SYSLOG:
		wrapper_syslog(LOG_ERR, "%s", srv->errorlog_buf->ptr);
		break;
	}
