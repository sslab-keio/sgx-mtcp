...
static int pipe_open(pipe_t *p, int fd SECURITY_DC) {
	...
	if (0 != pipe(newpipe)) {
		my_fprintf(stderr, "can't open pipe");
		return -1;
	}
	if (0 == fd) {
...
int proc_open(proc_handler_t *proc, const char *command) {
	...
	if (NULL == (shell = getenv(SHELLENV)) &&
		...
		my_fprintf(stderr, "One of %s,%%SystemRoot,%%windir is required", SHELLENV);
		return -1;
	}
	...
	if (FALSE == procok) {
		my_fprintf(stderr, "failed to CreateProcess: %s", cmdline->ptr);
		buffer_free(cmdline);
		return -1;
	}
	buffer_free(cmdline);
...
int proc_open(proc_handler_t *proc, const char *command) {
	...
	if (child == 0) {
		...
		my_fprintf(stderr, "failed to execute shell: %s -c %s: %s\n", shell, command, strerror(errno));
		exit(127);

	} else if (child < 0) {
		my_fprintf(stderr, "failed to forking");
		proc_close(proc);
		return -1;
...
#define RESET() do { \
	...
	if (0&&wstatus != 0) { \
		my_fprintf(stdout, "exitstatus %d\n", wstatus); \
		return __LINE__ - 200; \
	} \
} while (0)

#define ERROR_OUT() do { \
	my_fprintf(stdout, "failed opening proc\n"); \
	wstatus = proc_close(&proc); \
	my_fprintf(stdout, "exitstatus %d\n", wstatus); \
	FREE(); \
	return __LINE__ - 300; \
...
#define CMD_CAT "cat"
	...
	do {
		my_fprintf(stdout, "test: echo 123 without read\n");
		...
		my_fprintf(stdout, "test: echo 321 with read\n"); fflush(stdout);
		if (proc_open_buffer("echo 321", NULL, out, err) != 0) {
			ERROR_OUT();
		}
		my_fprintf(stdout, "result: ->%s<-\n\n", out->ptr); fflush(stdout);
		RESET();

		my_fprintf(stdout, "test: echo 123 | " CMD_CAT "\n"); fflush(stdout);
