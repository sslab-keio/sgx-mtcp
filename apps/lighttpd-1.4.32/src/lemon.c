...
struct action *Action_new(){
  ...
  if( freelist==NULL ){
    ...
    if( freelist==0 ){
      my_fprintf(stderr,"Unable to allocate memory for a new parser action.");
      exit(1);
    }
...
PRIVATE acttab *acttab_alloc(void){
  ...
  if( p==0 ){
    my_fprintf(stderr,"Unable to allocate memory for a new acttab.");
    exit(1);
  }
  memset(p, 0, sizeof(*p));
  return p;
...
int line;
{
  my_fprintf(stderr,"Assertion failed on line %d of file \"%s\"\n",line,file);
  exit(1);
}
...
PRIVATE struct config *newconfig(){
  ...
  if( freelist==0 ){
    ...
    if( freelist==0 ){
      my_fprintf(stderr,"Unable to allocate memory for a new configuration.");
      exit(1);
    }
    for(i=0; i<amt-1; i++) freelist[i].next = &freelist[i+1];
    freelist[amt-1].next = 0;
...
*/
void memory_error() {
  my_fprintf(stderr,"Out of memory.  Aborting...\n");
  exit(1);
}
...
char **argv;
{
  ...
  if( OptNArgs() < 1 ){
    my_fprintf(stderr,"Exactly one filename argument is required.\n");
    exit(1);
  }
  lem.errorcnt = 0;
  ...
  if( lem.rule==0 ){
    my_fprintf(stderr,"Empty grammar.\n");
    exit(1);
  }
  ...
  if( lem.nconflict ){
    my_fprintf(stderr,"%d parsing conflicts.\n",lem.nconflict);
  }
...
{
  int spcnt, i;
  if( argv[0] ) my_fprintf(err,"%s",argv[0]);
  spcnt = strlen(argv[0]) + 1;
  for(i=1; i<n && argv[i]; i++){
    my_fprintf(err," %s",argv[i]);
    spcnt += strlen(argv[i]+1);
  }
  spcnt += k;
  for(; argv[i]; i++) my_fprintf(err," %s",argv[i]);
  if( spcnt<20 ){
    my_fprintf(err,"\n%*s^-- here\n",spcnt,"");
  }else{
    my_fprintf(err,"\n%*shere --^\n",spcnt-7,"");
...
FILE *err;
{
  ...
  if( op[j].label==0 ){
    if( err ){
      my_fprintf(err,"%sundefined option.\n",emsg);
      errline(i,1,err);
    }
    errcnt++;
    ...
    *((int*)op[j].arg) = v;
    ...
    if( err ){
      my_fprintf(err,"%smissing argument on switch.\n",emsg);
      errline(i,1,err);
    }
    errcnt++;
  }
  ...
  *cp = '=';
  if( op[j].label==0 ){
    ...
    switch( op[j].type ){
      ...
      case OPT_FFLAG:
        if( err ){
          my_fprintf(err,"%soption requires an argument.\n",emsg);
          errline(i,0,err);
        }
        errcnt++;
        break;
      ...
      case OPT_FDBL:
        ...
        if( *end ){
          if( err ){
            my_fprintf(err,"%sillegal character in floating-point argument.\n",emsg);
      ...
      case OPT_FINT:
        ...
        if( *end ){
          if( err ){
            my_fprintf(err,"%sillegal character in integer argument.\n",emsg);
            errline(i,((unsigned long)end)-(unsigned long)argv[i],err);
          }
          errcnt++;
        }
...
FILE *err;
{
  ...
  if( errcnt>0 ){
    my_fprintf(err,"Valid command line options for \"%s\" are:\n",*a);
    OptPrint();
    exit(1);
  }
  return 0;
...
void OptPrint(){
  ...
  for(i=0; op[i].label; i++){
    switch( op[i].type ){
      case OPT_FLAG:
      case OPT_FFLAG:
        my_fprintf(errstream,"  -%-*s  %s\n",max,op[i].label,op[i].message);
        break;
      case OPT_INT:
      case OPT_FINT:
        my_fprintf(errstream,"  %s=<integer>%*s  %s\n",op[i].label,
          (int)(max-strlen(op[i].label)-9),"",op[i].message);
        break;
      case OPT_DBL:
      case OPT_FDBL:
        my_fprintf(errstream,"  %s=<real>%*s  %s\n",op[i].label,
          (int)(max-strlen(op[i].label)-6),"",op[i].message);
        break;
      case OPT_STR:
      case OPT_FSTR:
        my_fprintf(errstream,"  %s=<string>%*s  %s\n",op[i].label,
...
char *suffix;
{
  ...
  if( name==0 ){
    my_fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
	/* skip directory, JK */
...
char *mode;
{
  ...
  if( fp==0 && *mode=='w' ){
    my_fprintf(stderr,"Can't open file \"%s\".\n",lemp->outname);
    lemp->errorcnt++;
    return 0;
  }
  return fp;
...
struct config *cfp;
{
  ...
  int i;
  rp = cfp->rp;
  my_fprintf(fp,"%s ::=",rp->lhs->name);
  for(i=0; i<=rp->nrhs; i++){
    if( i==cfp->dot ) my_fprintf(fp," *");
    if( i==rp->nrhs ) break;
    my_fprintf(fp," %s",rp->rhs[i]->name);
  }
}
...
struct lemon *lemp;
{
  ...
  for(i=0; i<lemp->nterminal; i++){
    if( SetFind(set,i) ){
      my_fprintf(out,"%s%s",spacer,lemp->symbols[i]->name);
      spacer = " ";
    }
  }
  my_fprintf(out,"]\n");
...
{
  while( plp ){
    my_fprintf(out,"%12s%s (state %2d) ","",tag,plp->cfp->stp->index);
...
PRIVATE int PrintAction(struct action *ap, FILE *fp, int indent){
  ...
  switch( ap->type ){
    case SHIFT:
      my_fprintf(fp,"%*s shift  %d",indent,ap->sp->name,ap->x.stp->index);
      break;
    case REDUCE:
      my_fprintf(fp,"%*s reduce %d",indent,ap->sp->name,ap->x.rp->index);
      break;
    case ACCEPT:
      my_fprintf(fp,"%*s accept",indent,ap->sp->name);
      break;
    case ERROR:
      my_fprintf(fp,"%*s error",indent,ap->sp->name);
      break;
    case CONFLICT:
      my_fprintf(fp,"%*s reduce %-3d ** Parsing conflict **",
...
struct lemon *lemp;
{
  ...
  for(i=0; i<lemp->nstate; i++){
    ...
    for(ap=stp->ap; ap; ap=ap->next){
      if( PrintAction(ap,fp,30) ) my_fprintf(fp,"\n");
    }
    my_fprintf(fp,"\n");
  }
  fclose(fp);
    ...
    if( name ){
      for(i=0; line[i]; i++){
        if( line[i]=='P' && strncmp(&line[i],"Parse",5)==0
          ...
          if( i>iStart ) my_fprintf(out,"%.*s",i-iStart,&line[iStart]);
          my_fprintf(out,"%s",name);
          i += 4;
          iStart = i+1;
        }
      }
    }
    my_fprintf(out,"%s",&line[iStart]);
  }
}
...
struct lemon *lemp;
{
  ...
  if( tpltname==0 ){
    my_fprintf(stderr,"Can't find the parser driver template file \"%s\".\n",
    lemp->tmplname);
    lemp->errorcnt++;
    return 0;
  }
  in = fopen(tpltname,"r");
  if( in==0 ){
    my_fprintf(stderr,"Can't open the template file \"%s\".\n",lemp->tmplname);
    lemp->errorcnt++;
    return 0;
  }
  return in;
...
int *lineno;
{
 ...
 for(; *cp; cp++){
   if( *cp=='$' && cp[1]=='$' ){
     my_fprintf(out,"(yypminor->yy%d)",sp->dtnum);
     cp++;
     continue;
   }
   if( *cp=='\n' ) linecnt++;
     ...
     if( isalpha(*cp) && (cp==rp->code || (!isalnum(cp[-1]) && cp[-1]!='_')) ){
       ...
       *xp = 0;
       if( rp->lhsalias && strcmp(cp,rp->lhsalias)==0 ){
         my_fprintf(out,"yygotominor.yy%d",rp->lhs->dtnum);
         cp = xp;
         lhsused = 1;
       }else{
         for(i=0; i<rp->nrhs; i++){
           if( rp->rhsalias[i] && strcmp(cp,rp->rhsalias[i])==0 ){
             my_fprintf(out,"yymsp[%d].minor.yy%d",i-rp->nrhs+1,rp->rhs[i]->dtnum);
             cp = xp;
             used[i] = 1;
             break;
           }
 ...
 for(i=0; i<rp->nrhs; i++){
   if( rp->rhsalias[i] && !used[i] ){
     ...
     if( has_destructor(rp->rhs[i],lemp) ){
       my_fprintf(out,"  yy_destructor(%d,&yymsp[%d].minor);\n",
          rp->rhs[i]->index,i-rp->nrhs+1); (*lineno)++;
     }else{
       my_fprintf(out,"        /* No destructor defined for %s */\n",
        rp->rhs[i]->name);
        (*lineno)++;
     }
   }
...
int mhflag;                 /* True if generating makeheaders output */
{
  ...
  if( types==0 || stddt==0 ){
    my_fprintf(stderr,"Out of memory.\n");
    exit(1);
  }
  ...
  */
  ...
  lineno = *plineno;
  if( mhflag ){ my_fprintf(out,"#if INTERFACE\n"); lineno++; }
  my_fprintf(out,"#define %sTOKENTYPE %s\n",name,
    lemp->tokentype?lemp->tokentype:"void*");  lineno++;
  if( mhflag ){ my_fprintf(out,"#endif\n"); lineno++; }
  my_fprintf(out,"typedef union {\n"); lineno++;
  my_fprintf(out,"  %sTOKENTYPE yy0;\n",name); lineno++;
  for(i=0; i<arraysize; i++){
    if( types[i]==0 ) continue;
    my_fprintf(out,"  %s yy%d;\n",types[i],i+1); lineno++;
    free(types[i]);
  }
  my_fprintf(out,"  int yy%d;\n",lemp->errsym->dtnum); lineno++;
  free(stddt);
  free(types);
  my_fprintf(out,"} YYMINORTYPE;\n"); lineno++;
  *plineno = lineno;
}
...
int mhflag;     /* Output in makeheaders format if true */
{
  ...
  if( mhflag ){
    char *prefix;
    my_fprintf(out,"#if INTERFACE\n"); lineno++;
    if( lemp->tokenprefix ) prefix = lemp->tokenprefix;
    else                    prefix = "";
    for(i=1; i<lemp->nterminal; i++){
      my_fprintf(out,"#define %s%-30s %2d\n",prefix,lemp->symbols[i]->name,i);
      lineno++;
    }
    my_fprintf(out,"#endif\n"); lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);

  /* Generate the defines */
  my_fprintf(out,"/* \001 */\n");
  my_fprintf(out,"#define YYCODETYPE %s\n",
    minimum_size_type(0, lemp->nsymbol+5)); lineno++;
  my_fprintf(out,"#define YYNOCODE %d\n",lemp->nsymbol+1);  lineno++;
  my_fprintf(out,"#define YYACTIONTYPE %s\n",
...
"Illegal stack size: [%s].  The stack size should be an integer constant.",
    ...
    my_fprintf(out,"#define YYSTACKDEPTH %s\n",lemp->stacksize);  lineno++;
  }else{
    my_fprintf(out,"#define YYSTACKDEPTH 100\n");  lineno++;
  }
  if( mhflag ){
    my_fprintf(out,"#if INTERFACE\n"); lineno++;
  }
  ...
  if( lemp->arg && lemp->arg[0] ){
    ...
    my_fprintf(out,"#define %sARG_SDECL %s;\n",name,lemp->arg);  lineno++;
    my_fprintf(out,"#define %sARG_PDECL ,%s\n",name,lemp->arg);  lineno++;
    my_fprintf(out,"#define %sARG_FETCH %s = yypParser->%s\n",
                 name,lemp->arg,&lemp->arg[i]);  lineno++;
    my_fprintf(out,"#define %sARG_STORE yypParser->%s = %s\n",
                 name,&lemp->arg[i],&lemp->arg[i]);  lineno++;
  }else{
    my_fprintf(out,"#define %sARG_SDECL\n",name);  lineno++;
    my_fprintf(out,"#define %sARG_PDECL\n",name);  lineno++;
    my_fprintf(out,"#define %sARG_FETCH\n",name); lineno++;
    my_fprintf(out,"#define %sARG_STORE\n",name); lineno++;
  }
  if( mhflag ){
    my_fprintf(out,"#endif\n"); lineno++;
  }
  my_fprintf(out,"#define YYNSTATE %d\n",lemp->nstate);  lineno++;
  my_fprintf(out,"#define YYNRULE %d\n",lemp->nrule);  lineno++;
  my_fprintf(out,"#define YYERRORSYMBOL %d\n",lemp->errsym->index);  lineno++;
  my_fprintf(out,"#define YYERRSYMDT yy%d\n",lemp->errsym->dtnum);  lineno++;
  if( lemp->has_fallback ){
    my_fprintf(out,"#define YYFALLBACK 1\n");  lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  */
  ...
  my_fprintf(out,"static YYACTIONTYPE yy_action[] = {\n"); lineno++;
  n = acttab_size(pActtab);
  for(i=j=0; i<n; i++){
    int action = acttab_yyaction(pActtab, i);
    if( action<0 ) action = lemp->nsymbol + lemp->nrule + 2;
    if( j==0 ) my_fprintf(out," /* %5d */ ", i);
    my_fprintf(out, " %4d,", action);
    if( j==9 || i==n-1 ){
      my_fprintf(out, "\n"); lineno++;
      j = 0;
    }else{
  ...
  my_fprintf(out,"static YYCODETYPE yy_lookahead[] = {\n"); lineno++;
  for(i=j=0; i<n; i++){
    int la = acttab_yylookahead(pActtab, i);
    if( la<0 ) la = lemp->nsymbol;
    if( j==0 ) my_fprintf(out," /* %5d */ ", i);
    my_fprintf(out, " %4d,", la);
    if( j==9 || i==n-1 ){
      my_fprintf(out, "\n"); lineno++;
      j = 0;
    }else{
  ...
  my_fprintf(out, "#define YY_SHIFT_USE_DFLT (%d)\n", mnTknOfst-1); lineno++;
  my_fprintf(out, "static %s yy_shift_ofst[] = {\n",
  ...
  my_fprintf(out, "#define YY_REDUCE_USE_DFLT (%d)\n", mnNtOfst-1); lineno++;
  my_fprintf(out, "static %s yy_reduce_ofst[] = {\n",
  ...
  my_fprintf(out, "static YYACTIONTYPE yy_default[] = {\n"); lineno++;
  n = lemp->nstate;
  for(i=j=0; i<n; i++){
    stp = lemp->sorted[i];
    if( j==0 ) my_fprintf(out," /* %5d */ ", i);
    my_fprintf(out, " %4d,", stp->iDflt);
    if( j==9 || i==n-1 ){
      my_fprintf(out, "\n"); lineno++;
      j = 0;
    }else{
  ...
  */
  if( lemp->has_fallback ){
    for(i=0; i<lemp->nterminal; i++){
      ...
      if( p->fallback==0 ){
        my_fprintf(out, "    0,  /* %10s => nothing */\n", p->name);
      }else{
        my_fprintf(out, "  %3d,  /* %10s => %s */\n", p->fallback->index,
  ...
  */
  for(i=0; i<lemp->nsymbol; i++){
    ...
    my_fprintf(out,"  %-15s",line);
    if( (i&3)==3 ){ my_fprintf(out,"\n"); lineno++; }
  }
  if( (i&3)!=0 ){ my_fprintf(out,"\n"); lineno++; }
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  */
  for(i=0, rp=lemp->rule; rp; rp=rp->next, i++){
    assert( rp->index==i );
    my_fprintf(out," /* %3d */ \"%s ::=", i, rp->lhs->name);
    for(j=0; j<rp->nrhs; j++) my_fprintf(out," %s",rp->rhs[j]->name);
    my_fprintf(out,"\",\n"); lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);
  ...
  */
  if( lemp->tokendest ){
    for(i=0; i<lemp->nsymbol; i++){
      ...
      my_fprintf(out,"    case %d:\n",sp->index); lineno++;
    }
    for(i=0; i<lemp->nsymbol && lemp->symbols[i]->type!=TERMINAL; i++);
    if( i<lemp->nsymbol ){
      emit_destructor_code(out,lemp->symbols[i],lemp,&lineno);
      my_fprintf(out,"      break;\n"); lineno++;
    }
  }
  for(i=0; i<lemp->nsymbol; i++){
    struct symbol *sp = lemp->symbols[i];
