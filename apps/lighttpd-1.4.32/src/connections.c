...
#if 0
	my_fprintf(stderr, "%s.%d: add: ", __FILE__, __LINE__);
	for (i = 0; i < conns->used + 1; i++) {
		my_fprintf(stderr, "%d ", conns->ptr[i]->fd);
	}
	my_fprintf(stderr, "\n");
#endif
...
static int connection_del(server *srv, connection *con) {
	...
	con->ndx = -1;
#if 0
	my_fprintf(stderr, "%s.%d: del: (%d)", __FILE__, __LINE__, conns->used);
...
static void dump_packet(const unsigned char *data, size_t len) {
	...
	for (i = 0; i < len; i++) {
		if (i % 16 == 0) my_fprintf(stderr, "  ");

		my_fprintf(stderr, "%02x ", data[i]);
		...
		if ((i + 1) % 16 == 0) {
			...
			for (j = 0; j <= i % 16; j++) {
				...
				my_fprintf(stderr, "%c", c > 32 && c < 128 ? c : '.');
			}
