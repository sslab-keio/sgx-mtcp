...
#ifdef COMPILE_WITH_INTEL_SGX
thread_mutex_t logger_stack_lock = SGX_THREAD_MUTEX_INITIALIZER;
#else
thread_mutex_t logger_stack_lock = PTHREAD_MUTEX_INITIALIZER;
#endif

thread_key_t logger_key;

#if !defined(HAVE_GCC_64ATOMICS) && !defined(__sun)
#ifdef COMPILE_WITH_INTEL_SGX
thread_mutex_t logger_atomics_mutex = SGX_THREAD_MUTEX_INITIALIZER;
#else
thread_mutex_t logger_atomics_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif
#endif
...
static void logger_link_q(logger *l) {
    thread_mutex_lock(&logger_stack_lock);
    ...
    logger_count++;
    thread_mutex_unlock(&logger_stack_lock);
    return;
}
...
static void logger_set_flags(void) {
    ...
    for (l = logger_stack_head; l != NULL; l=l->next) {
        thread_mutex_lock(&l->mutex);
        l->eflags = f;
        thread_mutex_unlock(&l->mutex);
    }
    return;
}
...
static int logger_thread_read(logger *l, struct logger_stats *ls) {
    ...
    while (pos < size && watcher_count > 0) {
        ...
        if (ret != LOGGER_PARSE_ENTRY_OK) {
            /* TODO: stats counter */
            my_fprintf(stderr, "LOGGER: Failed to parse log entry\n");
        } else {
    ...
    thread_mutex_unlock(&l->mutex);
    if (data == NULL) {
        my_fprintf(stderr, "LOGGER: unexpectedly couldn't advance buf pointer\n");
        assert(0);
    }
...
static int logger_thread_poll_watchers(int force_poll, int watcher) {
    ...
    int ret = wrapper_poll(watchers_pollfds, nfd, 0);
...
static int start_logger_thread(void) {
    ...
    if ((ret = thread_create(&logger_tid, NULL,
                              logger_thread, NULL)) != 0) {
        my_fprintf(stderr, "Can't start logger thread: %s\n", strerror(ret));
        return -1;
    }
    return 0;
}
...
void logger_init(void) {
    ...
    logger_stack_head = 0;
    logger_stack_tail = 0;
    thread_key_create(&logger_key, NULL);
...
logger *logger_create(void) {
    ...
    thread_mutex_init(&l->mutex, NULL);
    thread_setspecific(logger_key, l);
...
enum logger_ret_type logger_log(logger *l, const enum log_entry_type event, const void *entry, ...) {
    ...
    switch (d->subtype) {
        case LOGGER_TEXT_ENTRY:
            ...
            if (total >= reqlen || total <= 0) {
                my_fprintf(stderr, "LOGGER: Failed to vsnprintf a text entry: (total) %d\n", total);
                break;
            }
    ...
    if (bipbuf_push(buf, (sizeof(logentry) + e->size + e->pad)) == 0) {
        my_fprintf(stderr, "LOGGER: Failed to bipbuf push a text entry\n");
        thread_mutex_unlock(&l->mutex);
