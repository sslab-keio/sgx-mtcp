...
#ifdef USE_MTCP
extern struct mtcp_conf mtcp_conf;
extern __thread mctx_t mctx;
extern __thread int listener;
extern __thread int ep;
#endif
...
#endif
typedef struct {
    thread_t thread_id;        /* unique ID of this thread */
...
#define mutex_lock(x) thread_mutex_lock(x)
#define mutex_unlock(x) thread_mutex_unlock(x)
