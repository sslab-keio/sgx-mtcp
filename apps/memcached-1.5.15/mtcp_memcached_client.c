...
void print_global_config(struct global_config *cfg) {
  ...
  printf("GET:UPDATE = %d:%d\n", cfg->get_rate, 100-cfg->get_rate);
}
/*----------------------------------------------------------------------------*/
...
static void print_statistics() {
  fprintf(stdout, "[ALL] GET: %lf Mbps, UPDATE: %lf Mbps, ", statistics.get, statistics.update);

  fprintf(stdout,
          "OPS: %lu kops, Avg "
          "Latency: %lf usec, Max Latency = %lf usec\n",
          statistics.kops, statistics.total_latency / 1000 / config.total_cores,
...
void print_per_thread_statistics(thread_context_t ctx) {
  ...
  double get, update;
  get = ctx->stat.get * 1000 * 1000 * 1000 * 8 / 1024.0 / 1024.0 /
         elapsed_nsec;  // Mbps

  update = ctx->stat.update * 1000 * 1000 * 1000 * 8 / 1024.0 / 1024.0 /
         elapsed_nsec;  // Mbps
  ...
  fprintf(stdout, "[CPU %d] GET: %lf Mbps, UPDATE: %lf Mbps, ", ctx->core, get, update);

  fprintf(stdout,
          "OPS: %lu kops, Avg "
          "Latency: %lf usec, Max Latency = %lf usec\n",
          kops, avg_latency / 1000, max_latency / 1000);
}
/*----------------------------------------------------------------------------*/
int main(int argc, char **argv) {
  ...
  config.dport = htons(11211);
  config.saddr = INADDR_ANY;
  config.long_connection = FALSE;

  // Options
  int opt;
  while (-1 != (opt = getopt(argc, argv, "N:c:n:f:v:k:r:l"))) {
    switch (opt) {
      case 'N':
      ...
      case 'f':
        ...
        break;
      case 'v':
        config.item_size = strtol(optarg, NULL, 10);
        if (config.item_size < 1 || config.item_size > MAX_BUFFER_SIZE - 128) {
          fprintf(stderr,
                  "Item size must be greater than 0 and less than %d!\n",
                  MAX_BUFFER_SIZE - 128);
          return FALSE;
        }
        break;
      case 'k':
        config.num_items = strtol(optarg, NULL, 10);
        break;
      case 'r':
        config.get_rate = strtol(optarg, NULL, 10);
        break;
      case 'l':
        config.long_connection = TRUE;
        break;
    }
  }
  ...
  ret = create_items();
  if (ret < 0) {
    fprintf(stderr, "Failed to create %d items.\n", config.num_items);
    exit(-1);
  }
...
/*----------------------------------------------------------------------------*/
int send_raw_bytes(thread_context_t ctx, int sockid, char *buffer, int len) {
  int ret;
  int total_sent = 0;

  do {
    ret = mtcp_write(ctx->mctx, sockid, buffer + total_sent, len - total_sent);
...
/*----------------------------------------------------------------------------*/
int send_new_item_request(thread_context_t ctx, int sockid,
                          uint64_t num_items_created) {
  int ret1, ret2, len;
  struct mtcp_epoll_event ev;
  char buffer[MAX_BUFFER_SIZE];

  // create key
  snprintf(buffer, MAX_BUFFER_SIZE, "set key_%010lu 1 0 %d\r\n",
           num_items_created, config.item_size);
  len = strnlen(buffer, MAX_BUFFER_SIZE);

  // send key
  ret1 = send_raw_bytes(ctx, sockid, buffer, len);
  assert(ret1 == len);
  if (ret1 < 0) {
    return ret1;
  }
  ...
  // create value
  memset(buffer, 'a', config.item_size);
  buffer[config.item_size] = '\r';
  buffer[config.item_size + 1] = '\n';

  // send value
  ret2 = send_raw_bytes(ctx, sockid, buffer, config.item_size + 2);
  assert(ret2 == config.item_size + 2);
  if (ret2 < 0) {
    return ret2;
  }
...
/*----------------------------------------------------------------------------*/
int recv_new_item_response(thread_context_t ctx, int sockid) {
  int ret;
  ...
  while (total_received < len) {
    ret = mtcp_read(ctx->mctx, sockid, buffer + total_received,
                    MAX_BUFFER_SIZE - total_received);
    if (ret == 0) {  // connection is closed by server
      return ret;
    } else if (ret < 0) {
      if (errno == EAGAIN) {
        break;
  ...
  if (strncmp(buffer, "STORED", strlen("STORED")) != 0) {
    printf("%s Received [%s] instead of [%s]\n", __func__, buffer, "STORED");
    return -1;
  }
...
/*----------------------------------------------------------------------------*/
int send_update_item_request(thread_context_t ctx, int sockid) {
  int ret1, ret2, len;
  struct mtcp_epoll_event ev;
  socket_context_t sctx = &ctx->sctxs[sockid];
  char buffer[MAX_BUFFER_SIZE];
  int offset = config.num_items / config.total_cores;
  // Avoid key contention
  uint64_t ikey = (rand() % offset) + (ctx->core * offset);

  // Operation start
  clock_gettime(CLOCK_MONOTONIC, &sctx->start_ts);

  // update key
  snprintf(buffer, MAX_BUFFER_SIZE, "replace key_%010lu 1 0 %d\r\n", ikey,
           config.item_size);
  len = strnlen(buffer, MAX_BUFFER_SIZE);
...
/*----------------------------------------------------------------------------*/
int recv_update_item_response(thread_context_t ctx, int sockid) {
  int ret;
  int len = strlen("STORED\r\n");
  socket_context_t sctx = &ctx->sctxs[sockid];
  struct mtcp_epoll_event ev;
  ...
  do {
    ret = mtcp_read(ctx->mctx, sockid, buf + sctx->total_recv,
                    MAX_BUFFER_SIZE - sctx->total_recv);
    ...
    // See set response format of memcached
    if (sctx->total_recv >= len) break;
  } while (ret > 0);

  if (strncmp(buf, "STORED", strlen("STORED")) != 0) {
    printf("%s Received [%s] instead of [%s]\n", __func__, buf, "STORED");
    return -1;
  }

  if (config.long_connection) {
    ev.events = MTCP_EPOLLOUT;
    ev.data.sockid = sockid;
    mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_MOD, sockid, &ev);
  } else {
  ...
  ctx->stat.update += config.item_size;
  ctx->stat.operations++;

  return ret;
}
/*----------------------------------------------------------------------------*/
int send_get_item_request(thread_context_t ctx, int sockid) {
  int ret, len;
  struct mtcp_epoll_event ev;
  socket_context_t sctx = &ctx->sctxs[sockid];
  char buf[MAX_BUFFER_SIZE];
  uint64_t ikey = rand() % config.num_items;

  // Operation start
  clock_gettime(CLOCK_MONOTONIC, &sctx->start_ts);

  snprintf(buf, MAX_BUFFER_SIZE, "get key_%010lu\r\n", ikey);
  len = strnlen(buf, MAX_BUFFER_SIZE);

  ret = send_raw_bytes(ctx, sockid, buf, len);
  if (ret < 0) {
    return ret;
  }
  assert(ret == len);
...
/*----------------------------------------------------------------------------*/
int recv_get_item_response(thread_context_t ctx, int sockid) {
  int ret;
  socket_context_t sctx = &ctx->sctxs[sockid];
  struct mtcp_epoll_event ev;
  char *buf = sctx->buf;
  ...
  do {
    ...
    // See get response format of memcached
    if (sctx->total_recv >= config.item_size + 36) break;
  } while (ret > 0);

  if (strncmp(buf + sctx->total_recv - 2 - strlen("END"), "END",
              strlen("END")) != 0) {
    printf("The buffer only contains [%s], length: %d\n", buf,
           sctx->total_recv);
    exit(EXIT_FAILURE);
  }
...
/*----------------------------------------------------------------------------*/
int create_items() {
  thread_context_t ctx = create_thread_context(0);
  ...
  int num_items_to_create = config.num_items;
  int num_items_requested = 0;
  ...
  while (ctx->connections != 0 && !config.state.done[core]) {
    ...
    for (int i = 0; i < nevents; i++) {
      if (events[i].events & MTCP_EPOLLERR) {
        ...
        int ret = recv_new_item_response(ctx, events[i].data.sockid);
        num_items_created++;

        if (ret < 0) {
          if (errno != EAGAIN) {  // EAGAIN means data hasn't arrived yet
            fprintf(stderr, "recv_new_item_request failed.\n");
            exit(EXIT_FAILURE);
          }
        } else if (ret == 0) {
        ...
        if (num_items_requested >= num_items_to_create) {
          close_connection(ctx, events[i].data.sockid);
          continue;
        }

        int ret = send_new_item_request(ctx, events[i].data.sockid,
                                        num_items_requested);
        num_items_requested++;
        ...
        if (ret < 0) {
          if (errno !=
            ...
            fprintf(stderr, "send_new_item_request failed.\n");
            exit(EXIT_FAILURE);
          }
        } else if (ret == 0) {
          fprintf(stderr, "Connection was closed by the server.\n");
          exit(EXIT_FAILURE);
        }
      } else {
  ...
  assert(num_items_created == num_items_to_create);
...
/*----------------------------------------------------------------------------*/
int should_get_or_update(int get_rate) {
  int random = (rand() % 100);
  if (random < get_rate)
    return TRUE;
  else
...
void main_loop(thread_context_t ctx) {
  ...
  while (!config.state.done[core]) {
    ...
    for (int i = 0; i < nevents; i++) {
      if (events[i].events & MTCP_EPOLLERR) {
        ...
        if (ctx->sctxs[events[i].data.sockid].is_get)
          ret = recv_get_item_response(ctx, events[i].data.sockid);
        else
          ret = recv_update_item_response(ctx, events[i].data.sockid);

        if (ret < 0) {
          if (errno != EAGAIN) {  // EAGAIN means data hasn't arrived yet
            fprintf(stderr, "recv_get_item_response failed.\n");
            exit(EXIT_FAILURE);
          }
        } else if (ret == 0) {
          fprintf(stderr, "Connection is closed by the server.\n");
        ...
        ctx->sctxs[events[i].data.sockid].is_get = should_get_or_update(config.get_rate);
        if (ctx->sctxs[events[i].data.sockid].is_get)
          ret = send_get_item_request(ctx, events[i].data.sockid);
        else
          ret = send_update_item_request(ctx, events[i].data.sockid);

        if (ret < 0) {
          if (errno !=
              EAGAIN) {  // EAGAIN means server side receive buffer is full
            fprintf(stderr, "send_get_item_request failed.\n");
            exit(EXIT_FAILURE);
          }
        } else if (ret == 0) {
          fprintf(stderr, "Connection was closed by the server.\n");
  ...
  uint64_t elapsed_nsec = get_elapsed_nsec(&ctx->start_ts, &ctx->end_ts);
  statistics.get += ctx->stat.get * 1000 * 1000 * 1000 * 8 / 1024.0 / 1024.0 /
                    elapsed_nsec;  // Mbps
  statistics.update += ctx->stat.update * 1000 * 1000 * 1000 * 8 / 1024.0 /
                       1024.0 / elapsed_nsec;  // Mbps
  statistics.kops += ctx->stat.operations * 1000 * 1000 / elapsed_nsec;
