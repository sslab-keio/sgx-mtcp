...
#define wrapper_getrlimit(resource, rlim) getrlimit(resource, rlim)
#define wrapper_setrlimit(resource, rlim) setrlimit(resource, rlim)
#define wrapper_recvfrom(sockfd, buf, len, flags, src_addr, addrlen) recvfrom(sockfd, buf, len, flags, src_addr, addrlen)
#define wrapper_stat(pathname, statbuf) stat(pathname, statbuf)
#define wrapper_lstat(path, sb) lstat(path, sb)
#endif
...
#ifdef USE_MTCP
static int new_socket(struct addrinfo *ai, mctx_t mctx);
#else
...
#ifdef USE_MTCP
static __thread conn *listen_conn = NULL;
#else
...
ssize_t tcp_read(conn *c, void *buf, size_t count) {
    assert (c != NULL);
#ifdef USE_MTCP
    ssize_t ret = mtcp_read(c->mctx, c->sfd, buf, count);
	 if (ret < 0) {
		perror("tcp_read: ");
	 }
	 return ret;
...
ssize_t tcp_sendmsg(conn *c, struct msghdr *msg, int flags) {
    assert (c != NULL);
#ifdef USE_MTCP
	 return mtcp_writev(c->mctx, c->sfd, msg->msg_iov, msg->msg_iovlen);
#else
...
ssize_t tcp_write(conn *c, void *buf, size_t count) {
    assert (c != NULL);
#ifdef USE_MTCP
    return mtcp_write(c->mctx, c->sfd, buf, count);
#else
...
extern thread_mutex_t conn_lock;

/* Connection timeout thread bits */
static thread_t conn_timeout_tid;
...
static void *conn_timeout_thread(void *arg) {
    ...
    while(1) {
        if (settings.verbose > 2)
            my_fprintf(stderr, "idle timeout thread at top of connection list\n");
        ...
        for (i = 0; i < max_fds; i++) {
            if ((i % CONNS_PER_SLICE) == 0) {
                if (settings.verbose > 2)
                    my_fprintf(stderr, "idle timeout thread sleeping for %ulus\n",
...
static int start_conn_timeout_thread() {
    ...
    if ((ret = thread_create(&conn_timeout_tid, NULL,
        conn_timeout_thread, NULL)) != 0) {
        my_fprintf(stderr, "Can't create idle connection timeout thread: %s\n",
            strerror(ret));
        return -1;
    }
...
static void conn_init(void) {
#ifdef USE_MTCP
    // this function has to be called by each thread
    // as conns is now local storageS
    max_fds = mtcp_conf.max_concurrency * 3;
#else
    ...
    if (wrapper_getrlimit(RLIMIT_NOFILE, &rl) == 0) {
        max_fds = rl.rlim_max;
    } else {
        my_fprintf(stderr, "Failed to query maximum file descriptor; "
    ...
    if ((conns = calloc(max_fds, sizeof(conn *))) == NULL) {
        my_fprintf(stderr, "Failed to allocate connection structures\n");
...
void conn_close_idle(conn *c) {
    if (settings.idle_timeout > 0 &&
        ...
        if (settings.verbose > 1)
            my_fprintf(stderr, "Closing idle fd %d\n", c->sfd);
...
#ifdef USE_MTCP
    event_handler(c->sfd, c->ev_flags, (void*)c);
#else
...
conn *conn_retrieve_conn(const int sfd, struct event_base *base) {
	conn *c = NULL;
	if (conns[sfd] == NULL || conns[sfd]->sfd != sfd || conns[sfd]->state == conn_closed) {
		c = conn_new(sfd, conn_new_cmd, EV_READ | EV_PERSIST,
				DATA_BUFFER_SIZE, tcp_transport, base, NULL);
	} else {
		c = conns[sfd];
	}
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {
        if (!(c = (conn *)calloc(1, sizeof(conn)))) {
            ...
            stats.malloc_fails++;
            STATS_UNLOCK();
            my_fprintf(stderr, "Failed to allocate connection object\n");
            return NULL;
        }
...
#ifdef COMPILE_WITH_INTEL_SGX
        c->rbuf = (char *)untrusted_malloc((size_t)c->rsize);
        c->wbuf = (char *)untrusted_malloc((size_t)c->wsize);
        c->ilist = (item **)untrusted_malloc(sizeof(item *) * c->isize);
        c->suffixlist = (char **)untrusted_malloc(sizeof(char *) * c->suffixsize);
        c->iov = (struct iovec *)untrusted_malloc(sizeof(struct iovec) * c->iovsize);
        c->msglist = (struct msghdr *)untrusted_malloc(sizeof(struct msghdr) * c->msgsize);
#else
        ...
        if (c->rbuf == 0 || c->wbuf == 0 || c->ilist == 0 || c->iov == 0 ||
            ...
            stats.malloc_fails++;
            STATS_UNLOCK();
            my_fprintf(stderr, "Failed to allocate buffers for connection\n");
            return NULL;
        }
    ...
    if (settings.verbose > 1) {
        if (init_state == conn_listening) {
            my_fprintf(stderr, "<%d server listening (%s)\n", sfd,
                prot_text(c->protocol));
        } else if (IS_UDP(transport)) {
            my_fprintf(stderr, "<%d server listening (udp)\n", sfd);
        } else if (c->protocol == negotiating_prot) {
            my_fprintf(stderr, "<%d new auto-negotiating client connection\n",
                    sfd);
        } else if (c->protocol == ascii_prot) {
            my_fprintf(stderr, "<%d new ascii client connection.\n", sfd);
        } else if (c->protocol == binary_prot) {
            my_fprintf(stderr, "<%d new binary client connection.\n", sfd);
        } else {
            my_fprintf(stderr, "<%d new unknown (%d) client connection\n",
                sfd, c->protocol);
            assert(false);
        }
    }
...
static void recache_or_free(conn *c, io_wrap *wrap) {
    ...
    if (wrap->active) {
        ...
        thread_mutex_lock(&c->thread->stats.mutex);
        c->thread->stats.get_aborted_extstore++;
        thread_mutex_unlock(&c->thread->stats.mutex);
    } else if (wrap->miss) {
...
static void conn_close(conn *c) {
    ...
    if (settings.verbose > 1)
        my_fprintf(stderr, "<%d connection closed.\n", c->sfd);
...
#endif
#ifdef USE_MTCP
    mtcp_close(c->mctx, c->sfd);
#else
    close(c->sfd);
#endif
    thread_mutex_lock(&conn_lock);
    allow_new_conns = true;
    thread_mutex_unlock(&conn_lock);
...
static void conn_set_state(conn *c, enum conn_states state) {
    ...
    if (state != c->state) {
        if (settings.verbose > 2) {
            my_fprintf(stderr, "%d: going from %s to %s\n",
        ...
        c->state = state;
		  ...
		  ev.data.sockid = c->sfd;
		  mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_MOD, c->sfd, &ev);
#endif
    }
}
...
static void add_bin_header(conn *c, uint16_t err, uint8_t hdr_len, uint16_t key_len, uint32_t body_len) {
    ...
    if (settings.verbose > 1) {
        int ii;
        my_fprintf(stderr, ">%d Writing bin response:", c->sfd);
        for (ii = 0; ii < sizeof(header->bytes); ++ii) {
            if (ii % 4 == 0) {
                my_fprintf(stderr, "\n>%d  ", c->sfd);
            }
            my_fprintf(stderr, " 0x%02x", header->bytes[ii]);
        }
        my_fprintf(stderr, "\n");
    }
...
static void write_bin_error(conn *c, protocol_binary_response_status err,
    ...
    if (!errstr) {
        ...
        default:
            ...
            my_fprintf(stderr, ">%d UNHANDLED ERROR: %d\n", c->sfd, err);
        }
    }

    if (settings.verbose > 1) {
        my_fprintf(stderr, ">%d Writing an error: %s\n", c->sfd, errstr);
    }
...
static void complete_incr_bin(conn *c) {
    ...
    if (settings.verbose > 1) {
        int i;
        my_fprintf(stderr, "incr ");

        for (i = 0; i < nkey; i++) {
            my_fprintf(stderr, "%c", key[i]);
        }
        my_fprintf(stderr, " %lld, %llu, %d\n",
...
static void process_bin_get_or_touch(conn *c) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "<%d %s ", c->sfd, should_touch ? "TOUCH" : "GET");
...
static void process_bin_stat(conn *c) {
    ...
    if (settings.verbose > 1) {
        int ii;
        my_fprintf(stderr, "<%d STATS ", c->sfd);
        for (ii = 0; ii < nkey; ++ii) {
            my_fprintf(stderr, "%c", subcommand[ii]);
        }
        my_fprintf(stderr, "\n");
    }
...
static void bin_read_key(conn *c, enum bin_substates next_substate, int extra) {
    ...
    if (c->rlbytes > c->rsize - offset) {
        ...
        if (nsize != c->rsize) {
            if (settings.verbose > 1) {
                my_fprintf(stderr, "%d: Need to grow buffer from %lu to %lu\n",
            ...
            if (newm == NULL) {
                ...
                STATS_UNLOCK();
                if (settings.verbose) {
                    my_fprintf(stderr, "%d: Failed to grow buffer.. closing connection\n",
                            c->sfd);
                }
                conn_set_state(c, conn_closing);
                return;
        ...
        if (c->rbuf != c->rcurr) {
            ...
            c->rcurr = c->rbuf;
            if (settings.verbose > 1) {
                my_fprintf(stderr, "%d: Repack input buffer\n", c->sfd);
            }
        }
    }
...
static void handle_binary_protocol_error(conn *c) {
    ...
    if (settings.verbose) {
        my_fprintf(stderr, "Protocol error (opcode %02x), close connection %d\n",
...
static void init_sasl_conn(conn *c) {
    ...
    if (!c->sasl_conn) {
        ...
        if (result != SASL_OK) {
            if (settings.verbose) {
                my_fprintf(stderr, "Failed to initialize SASL conn.\n");
            }
            c->sasl_conn = NULL;
        }
    }
...
static void bin_list_sasl_mechs(conn *c) {
    ...
    if (result != SASL_OK) {
        ...
        if (settings.verbose) {
            my_fprintf(stderr, "Failed to list SASL mechanisms.\n");
        }
...
static void process_bin_complete_sasl_auth(conn *c) {
    ...
    if (settings.verbose)
        my_fprintf(stderr, "mech:  ``%s'' with %d bytes of data\n", mech, vlen);
    ...
    case PROTOCOL_BINARY_CMD_SASL_STEP:
        if (!c->sasl_started) {
            if (settings.verbose) {
                my_fprintf(stderr, "%d: SASL_STEP called but sasl_server_start "
    ...
    default:
        ...
        /* This code is pretty much impossible, but makes the compiler
           happier */
        if (settings.verbose) {
            my_fprintf(stderr, "Unhandled command %d with challenge %s\n",
                    c->cmd, challenge);
        }
        break;
    }
    ...
    if (settings.verbose) {
        my_fprintf(stderr, "sasl result code:  %d\n", result);
    }
    ...
    default:
        if (settings.verbose)
            my_fprintf(stderr, "Unknown sasl response:  %d\n", result);
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_AUTH_ERROR, NULL, 0);
        thread_mutex_lock(&c->thread->stats.mutex);
        c->thread->stats.auth_cmds++;
...
static bool authenticated(conn *c) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "authenticated() in cmd 0x%02x is %s\n",
                c->cmd, rv ? "true" : "false");
    }
...
static void process_bin_update(conn *c) {
    ...
    if (settings.verbose > 1) {
        ...
        if (c->cmd == PROTOCOL_BINARY_CMD_ADD) {
            my_fprintf(stderr, "<%d ADD ", c->sfd);
        } else if (c->cmd == PROTOCOL_BINARY_CMD_SET) {
            my_fprintf(stderr, "<%d SET ", c->sfd);
        } else {
            my_fprintf(stderr, "<%d REPLACE ", c->sfd);
        }
        for (ii = 0; ii < nkey; ++ii) {
            my_fprintf(stderr, "%c", key[ii]);
        }

        my_fprintf(stderr, " Value len is %d", vlen);
        my_fprintf(stderr, "\n");
    }
...
static void process_bin_append_prepend(conn *c) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "Value len is %d\n", vlen);
    }
...
static void complete_nread_binary(conn *c) {
    ...
    case bin_reading_sasl_auth_data:
        ...
        break;
    default:
        my_fprintf(stderr, "Not handling substate %d\n", c->substate);
        assert(0);
    }
}
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if(old_it == NULL) {
            ...
            if(settings.verbose > 1) {
                my_fprintf(stderr, "CAS:  failure: expected %llu, got %llu\n",
...
static inline int _get_extstore(conn *c, item *it, int iovst, int iovcnt) {
    ...
    //my_fprintf(stderr, "EXTSTORE: IO stacked %u\n", io->iovec_data);
    ...
    do {
        while(key_token->length != 0) {
            ...
            if (it) {
                ...
                if (settings.verbose > 1) {
                    int ii;
                    my_fprintf(stderr, ">%d sending key ", c->sfd);
...
enum delta_result_type do_add_delta(conn *c, const char *key, const size_t nkey,
    ...
    if (res + 2 <= it->nbytes && it->refcount == 2) { /* replace in-place */
        ...
        if (settings.verbose) {
            my_fprintf(stderr, "Tried to do incr/decr on invalid item\n");
        }
        if (it->refcount == 1)
            do_item_remove(it);
        return DELTA_ITEM_NOT_FOUND;
        ...
        if (memlimit < 8) {
            ...
            if (memlimit > 1000000000) {
                ...
                if (settings.verbose > 0) {
                    my_fprintf(stderr, "maxbytes adjusted to %llum\n", (unsigned long long)memlimit);
                }
...
static int try_read_command_negotiate(conn *c) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "%d: Client using the %s protocol\n", c->sfd,
                prot_text(c->protocol));
    }
...
static int try_read_command_binary(conn *c) {
    ...
    if (c->rbytes < sizeof(c->binary_header)) {
        ...
        if (((long)(c->rcurr)) % 8 != 0) {
            ...
            c->rcurr = c->rbuf;
            if (settings.verbose > 1) {
                my_fprintf(stderr, "%d: Realign input buffer\n", c->sfd);
            }
        }
#endif
        ...
        if (settings.verbose > 1) {
            ...
            int ii;
            my_fprintf(stderr, "<%d Read binary protocol data:", c->sfd);
            for (ii = 0; ii < sizeof(req->bytes); ++ii) {
                if (ii % 4 == 0) {
                    my_fprintf(stderr, "\n<%d   ", c->sfd);
                }
                my_fprintf(stderr, " 0x%02x", req->bytes[ii]);
            }
            my_fprintf(stderr, "\n");
        }
        ...
        if (c->binary_header.request.magic != PROTOCOL_BINARY_REQ) {
            if (settings.verbose) {
                my_fprintf(stderr, "Invalid magic:  %x\n",
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    c->request_addr_size = sizeof(c->request_addr);
    res = wrapper_recvfrom(c->sfd, c->rbuf, c->rsize,
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            if (!new_rbuf) {
                ...
                STATS_UNLOCK();
                if (settings.verbose > 0) {
                    my_fprintf(stderr, "Couldn't realloc input buffer\n");
                }
    ...
    if (c->msgcurr < c->msgused) {
        ...
        if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            if (!update_event(c, EV_WRITE | EV_PERSIST)) {
                if (settings.verbose > 0)
                    my_fprintf(stderr, "Couldn't update event\n");
                conn_set_state(c, conn_closing);
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_listening:
#ifdef USE_MTCP
	        sfd = mtcp_accept(c->mctx, c->sfd, NULL, NULL);
#else
            addrlen = sizeof(addr);
#ifdef HAVE_ACCEPT4
            if (use_accept4) {
            ...
            if (sfd == -1) {
                ...
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    ...
                    if (settings.verbose > 0)
                        my_fprintf(stderr, "Too many open connections\n");
...
#ifdef USE_MTCP
            mtcp_setsock_nonblock(c->mctx, sfd);
#else
            if (!use_accept4) {
            ...
            if (settings.maxconns_fast &&
...
#ifdef USE_MTCP
                mtcp_write(c->mctx, sfd, str, strlen(str));
                mtcp_close(c->mctx, sfd);
#else
                ...
                if (c->ssl_enabled) {
                    ...
                    if (settings.ssl_ctx == NULL) {
                        if (settings.verbose) {
                            my_fprintf(stderr, "SSL context is not initialized\n");
                        }
                        close(sfd);
                        break;
                    }
                    ...
                    if (ssl == NULL) {
                        if (settings.verbose) {
                            my_fprintf(stderr, "Failed to created the SSL object\n");
                        }
                        close(sfd);
                        break;
                    }
                    ...
                    if (ret < 0) {
                        ...
                        if (err == SSL_ERROR_SYSCALL || err == SSL_ERROR_SSL) {
                            if (settings.verbose) {
                                my_fprintf(stderr, "SSL connection failed with error code : %d : %s\n", err, strerror(errno));
                            }
                            close(sfd);
                            break;
                        }
        ...
        case conn_nread:
            ...
            if (c->rlbytes < 0) {
                if (settings.verbose) {
                    my_fprintf(stderr, "Invalid rlbytes to read: len %d\n", c->rlbytes);
                }
                conn_set_state(c, conn_closing);
                break;
            }
            ...
            if (settings.verbose > 0) {
                my_fprintf(stderr, "Failed to read, and not due to blocking:\n"
                        "errno: %d %s \n"
        ...
        case conn_write:
            ...
            if (c->iovused == 0 || (IS_UDP(c->transport) && c->iovused == 1)) {
                if (add_iov(c, c->wcurr, c->wbytes) != 0) {
                    if (settings.verbose > 0)
                        my_fprintf(stderr, "Couldn't build response\n");
                    conn_set_state(c, conn_closing);
                    break;
                }
            }
        ...
        case conn_mwrite:
          ...
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            if (settings.verbose > 0)
              my_fprintf(stderr, "Failed to build UDP headers\n");
            conn_set_state(c, conn_closing);
            break;
          }
            switch (transmit(c)) {
            case TRANSMIT_COMPLETE:
                if (c->state == conn_mwrite) {
                    ...
                    if (settings.verbose > 0)
                        my_fprintf(stderr, "Unexpected state %d\n", c->state);
                    conn_set_state(c, conn_closing);
                }
                break;
...
void event_handler(const int fd, const short which, void *arg) {
    ...
    if (fd != c->sfd) {
        if (settings.verbose > 0)
            my_fprintf(stderr, "Catastrophic: event fd doesn't match conn fd!\n");
        conn_close(c);
        return;
    }
...
#ifdef USE_MTCP
   sfd = mtcp_socket(mctx, AF_INET, SOCK_STREAM, 0);
	if (sfd < 0) {
        perror("Failed to create listening socket!\n");
		return -1;
	}
	flags = mtcp_setsock_nonblock(mctx, sfd);
	if (flags < 0) {
...
static void maximize_sndbuf(const int sfd) {
    ...
    if (settings.verbose > 1)
        my_fprintf(stderr, "<%d send buffer was %d, now %d\n", sfd, old_size, last_good);
}
#endif
...
int server_socket(const char *interface,
    ...
    if (error != 0) {
        if (error != EAI_SYSTEM)
          my_fprintf(stderr, "getaddrinfo(): %s\n", gai_strerror(error));
        else
          perror("getaddrinfo()");
        return 1;
    }
    ...
    for (next= ai; next; next= next->ai_next) {
        ...
        saddr.sin_port = htons(settings.port);
        ret = mtcp_bind(mctx, sfd, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in));
#else
        ret = bind(sfd, next->ai_addr, next->ai_addrlen);
#endif
        if (ret == -1) {
            if (errno != EADDRINUSE) {
                perror("bind()");
            ...
            if (!IS_UDP(transport) &&
#ifdef USE_MTCP
                mtcp_listen(mctx, sfd, settings.backlog)
#else
            ...
            if (portnumber_file != NULL &&
                ...
                if (getsockname(sfd, (struct sockaddr*)&my_sockaddr, &len)==0) {
                    if (next->ai_addr->sa_family == AF_INET) {
                        my_fprintf(portnumber_file, "%s INET: %u\n",
                                IS_UDP(transport) ? "UDP" : "TCP",
                                ntohs(my_sockaddr.in.sin_port));
                    } else {
                        my_fprintf(portnumber_file, "%s INET6: %u\n",
                                IS_UDP(transport) ? "UDP" : "TCP",
        ...
        if (IS_UDP(transport)) {
            ...
            if (!(listen_conn_add = conn_new(sfd, conn_listening,
                ...
                my_fprintf(stderr, "failed to create listening connection\n");
                exit(EXIT_FAILURE);
            }
#ifdef TLS
...
#ifdef USE_MTCP
        break; // no need for another loop
#endif
    }
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        if (list == NULL) {
            my_fprintf(stderr, "Failed to allocate memory for parsing server interface string\n");
            return 1;
        }
        for (char *p = strtok_r(list, ";,", &b);
            ...
            if (strncmp(p, notls, strlen(notls)) == 0) {
                if (!settings.ssl_enabled) {
                    my_fprintf(stderr, "'notls' option is valid only when SSL is enabled\n");
                    return 1;
                }
                ssl_enabled = false;
                p += strlen(notls) + 1;
            ...
            if (*p == '[') {
                ...
                char *e = strchr(p, ']');
                if (e == NULL) {
                    my_fprintf(stderr, "Invalid IPV6 address: \"%s\"", p);
                    free(list);
                    return 1;
                }
                ...
                if (strchr(s + 1, ':') == NULL || h != NULL) {
                    *s = '\0';
                    ...
                    if (!safe_strtol(s, &the_port)) {
                        my_fprintf(stderr, "Invalid port number: \"%s\"", s);
                        free(list);
                        return 1;
                    }
                }
        ...
        if ((fp = fopen(pid_file, "r")) != NULL) {
            ...
            if (fgets(buffer, sizeof(buffer), fp) != NULL) {
                ...
                if (safe_strtoul(buffer, &pid) && kill((pid_t)pid, 0) == 0) {
                    my_fprintf(stderr, "WARNING: The pid file contained the following (running) pid: %u\n", pid);
                }
            }
            fclose(fp);
        }
...
static int enable_large_pages(void) {
    ...
    if (avail != -1) {
        ...
        if (memcntl(0, 0, MC_HAT_ADVISE, (caddr_t)&arg, 0, 0) == -1) {
            my_fprintf(stderr, "Failed to set large pages: %s\n",
                    strerror(errno));
            my_fprintf(stderr, "Will use default page size\n");
        } else {
            ret = 0;
        }
    } else {
        my_fprintf(stderr, "Failed to get supported pagesizes: %s\n",
                strerror(errno));
        my_fprintf(stderr, "Will use default page size\n");
    }
    ...
    struct stat st;
    int ret = wrapper_stat("/sys/kernel/mm/transparent_hugepage/enabled", &st);
    if (ret || !(st.st_mode & S_IFREG)) {
        my_fprintf(stderr, "Transparent huge pages support not detected.\n");
        my_fprintf(stderr, "Will use default page size.\n");
        return -1;
    }
    return 0;
    ...
    &spagesl, NULL, 0) != 0) {
        my_fprintf(stderr, "Could not evaluate the presence of superpages features.");
        return -1;
    }
    if (spages != 1) {
        my_fprintf(stderr, "Superpages support not detected.\n");
        my_fprintf(stderr, "Will use default page size.\n");
        return -1;
    }
    return 0;
    ...
    if (ever != NULL) {
        if (strncmp(ever, "1.", 2) == 0) {
            ...
            if (('0' <= ever[2] && ever[2] < '3') && !isdigit(ever[3])) {
                my_fprintf(stderr, "You are using libevent %s.\nPlease upgrade to"
...
static bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
    ...
    for (char *p = strtok_r(s, "-", &b);
        ...
        if (!safe_strtoul(p, &size) || size < settings.chunk_size
            ...
            my_fprintf(stderr, "slab size %u is out of valid range\n", size);
            return false;
        }
        if (last_size >= size) {
            my_fprintf(stderr, "slab size %u cannot be lower than or equal to a previous class size\n", size);
            return false;
        }
        if (size <= last_size + CHUNK_ALIGN_BYTES) {
            my_fprintf(stderr, "slab size %u must be at least %d bytes larger than previous class\n",
        ...
        if (i >= MAX_NUMBER_OF_SLAB_CLASSES-1) {
            my_fprintf(stderr, "too many slab classes specified\n");
            return false;
        }
    }
...
#ifdef COMPILE_WITH_INTEL_SGX
int
ecall_main_wrapper(int argc, char **argv)
{
...
int main_wrapper (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
...
#else
            my_fprintf(stderr, "This server is not built with TLS support.\n");
            exit(EX_USAGE);
#endif
            break;
        case 'a':
        ...
        case 'c':
            ...
            if (settings.maxconns <= 0) {
                my_fprintf(stderr, "Maximum connections must be greater than 0\n");
                return 1;
            }
            break;
        case 'h':
        ...
        case 'R':
            ...
            if (settings.reqs_per_event == 0) {
                my_fprintf(stderr, "Number of requests per event must be greater than 0\n");
                return 1;
            }
            break;
        case 'u':
        ...
        case 'f':
            ...
            if (settings.factor <= 1.0) {
                my_fprintf(stderr, "Factor must be greater than 1\n");
                return 1;
            }
            break;
        case 'n':
            ...
            if (settings.chunk_size == 0) {
                my_fprintf(stderr, "Chunk size must be greater than 0\n");
                return 1;
            }
            break;
        case 't':
            ...
            if (settings.num_threads <= 0) {
                my_fprintf(stderr, "Number of threads must be greater than 0\n");
                return 1;
            }
            ...
            if (settings.num_threads > 64) {
                my_fprintf(stderr, "WARNING: Setting a high number of worker"
        ...
        case 'D':
            if (! optarg || ! optarg[0]) {
                my_fprintf(stderr, "No delimiter specified\n");
                return 1;
            }
        ...
        case 'L' :
            if (enable_large_pages() == 0) {
                ...
                my_fprintf(stderr, "Cannot enable large pages on this system\n"
        ...
        case 'B':
            ...
            if (strcmp(optarg, "auto") == 0) {
                ...
                my_fprintf(stderr, "Invalid value for binding protocol: %s\n"
...
#ifndef ENABLE_SASL
            my_fprintf(stderr, "This server is not built with SASL support.\n");
            exit(EX_USAGE);
#endif
            settings.sasl = true;
            break;
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case HASHPOWER_INIT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing numeric argument for hashpower\n");
                    return 1;
                }
                settings.hashpower_init = atoi(subopts_value);
                if (settings.hashpower_init < 12) {
                    my_fprintf(stderr, "Initial hashtable multiplier of %d is too low\n",
                        settings.hashpower_init);
                    return 1;
                } else if (settings.hashpower_init > 32) {
                    my_fprintf(stderr, "Initial hashtable multiplier of %d is too high\n"
            ...
            case SLAB_AUTOMOVE:
                ...
                if (settings.slab_automove < 0 || settings.slab_automove > 2) {
                    my_fprintf(stderr, "slab_automove must be between 0 and 2\n");
                    return 1;
                }
                break;
            case SLAB_AUTOMOVE_RATIO:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing slab_automove_ratio argument\n");
                    return 1;
                }
                settings.slab_automove_ratio = atof(subopts_value);
                if (settings.slab_automove_ratio <= 0 || settings.slab_automove_ratio > 1) {
                    my_fprintf(stderr, "slab_automove_ratio must be > 0 and < 1\n");
                    return 1;
                }
                break;
            case SLAB_AUTOMOVE_WINDOW:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing slab_automove_window argument\n");
                    return 1;
                }
                settings.slab_automove_window = atoi(subopts_value);
                if (settings.slab_automove_window < 3) {
                    my_fprintf(stderr, "slab_automove_window must be > 2\n");
                    return 1;
                }
                break;
            case TAIL_REPAIR_TIME:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing numeric argument for tail_repair_time\n");
                    return 1;
                }
                settings.tail_repair_time = atoi(subopts_value);
                if (settings.tail_repair_time < 10) {
                    my_fprintf(stderr, "Cannot set tail_repair_time to less than 10 seconds\n");
                    return 1;
                }
                break;
            case HASH_ALGORITHM:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing hash_algorithm argument\n");
                    return 1;
                };
                if (strcmp(subopts_value, "jenkins") == 0) {
                    ...
                    my_fprintf(stderr, "Unknown hash_algorithm option (jenkins, murmur3)\n");
                    return 1;
                }
                break;
            case LRU_CRAWLER:
            ...
            case LRU_CRAWLER_SLEEP:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing lru_crawler_sleep value\n");
                    return 1;
                }
                settings.lru_crawler_sleep = atoi(subopts_value);
                if (settings.lru_crawler_sleep > 1000000 || settings.lru_crawler_sleep < 0) {
                    my_fprintf(stderr, "LRU crawler sleep must be between 0 and 1 second\n");
                    return 1;
                }
                break;
            case LRU_CRAWLER_TOCRAWL:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing lru_crawler_tocrawl value\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &tocrawl)) {
                    my_fprintf(stderr, "lru_crawler_tocrawl takes a numeric 32bit value\n");
                    return 1;
                }
                settings.lru_crawler_tocrawl = tocrawl;
                break;
            ...
            case HOT_LRU_PCT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing hot_lru_pct argument\n");
                    return 1;
                }
                settings.hot_lru_pct = atoi(subopts_value);
                if (settings.hot_lru_pct < 1 || settings.hot_lru_pct >= 80) {
                    my_fprintf(stderr, "hot_lru_pct must be > 1 and < 80\n");
                    return 1;
                }
                break;
            case WARM_LRU_PCT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing warm_lru_pct argument\n");
                    return 1;
                }
                settings.warm_lru_pct = atoi(subopts_value);
                if (settings.warm_lru_pct < 1 || settings.warm_lru_pct >= 80) {
                    my_fprintf(stderr, "warm_lru_pct must be > 1 and < 80\n");
                    return 1;
                }
                break;
            case HOT_MAX_FACTOR:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing hot_max_factor argument\n");
                    return 1;
                }
                settings.hot_max_factor = atof(subopts_value);
                if (settings.hot_max_factor <= 0) {
                    my_fprintf(stderr, "hot_max_factor must be > 0\n");
                    return 1;
                }
                break;
            case WARM_MAX_FACTOR:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing warm_max_factor argument\n");
                    return 1;
                }
                settings.warm_max_factor = atof(subopts_value);
                if (settings.warm_max_factor <= 0) {
                    my_fprintf(stderr, "warm_max_factor must be > 0\n");
                    return 1;
                }
                break;
            case TEMPORARY_TTL:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing temporary_ttl argument\n");
                    return 1;
                }
                settings.temp_lru = true;
            ...
            case IDLE_TIMEOUT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing numeric argument for idle_timeout\n");
                    return 1;
                }
            ...
            case WATCHER_LOGBUF_SIZE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing watcher_logbuf_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.logger_watcher_buf_size)) {
                    my_fprintf(stderr, "could not parse argument to watcher_logbuf_size\n");
                    return 1;
                }
            ...
            case WORKER_LOGBUF_SIZE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing worker_logbuf_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.logger_buf_size)) {
                    my_fprintf(stderr, "could not parse argument to worker_logbuf_size\n");
                    return 1;
                }
            ...
            case SLAB_CHUNK_MAX:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing slab_chunk_max argument\n");
                }
                if (!safe_strtol(subopts_value, &settings.slab_chunk_size_max)) {
                    my_fprintf(stderr, "could not parse argument to slab_chunk_max\n");
                }
            ...
            case SSL_CERT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_chain_cert argument\n");
                    return 1;
                }
            ...
            case SSL_KEY:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_key argument\n");
                    return 1;
                }
            ...
            {
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_verify_mode argument\n");
                    return 1;
                }
                int verify  = 0;
                if (!safe_strtol(subopts_value, &verify)) {
                    my_fprintf(stderr, "could not parse argument to ssl_verify_mode\n");
                    return 1;
                }
                switch(verify) {
                    case 0:
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        my_fprintf(stderr, "Invalid ssl_verify_mode. Use help to see valid options.\n");
                        return 1;
                }
                break;
            }
            case SSL_KEYFORM:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_keyformat argument\n");
                    return 1;
                }
                if (!safe_strtol(subopts_value, &settings.ssl_keyformat)) {
                    my_fprintf(stderr, "could not parse argument to ssl_keyformat\n");
                    return 1;
                }
                break;
            case SSL_CIPHERS:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_ciphers argument\n");
                    return 1;
                }
            ...
            case SSL_CA_CERT:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_ca_cert argument\n");
                    return 1;
                }
            ...
            case SSL_WBUF_SIZE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ssl_wbuf_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ssl_wbuf_size)) {
                    my_fprintf(stderr, "could not parse argument to ssl_wbuf_size\n");
                    return 1;
                }
            ...
            case EXT_PAGE_SIZE:
                if (storage_file) {
                    my_fprintf(stderr, "Must specify ext_page_size before any ext_path arguments\n");
                    return 1;
                }
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_page_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &ext_cf.page_size)) {
                    my_fprintf(stderr, "could not parse argument to ext_page_size\n");
                    return 1;
                }
            ...
            case EXT_WBUF_SIZE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_wbuf_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &ext_cf.wbuf_size)) {
                    my_fprintf(stderr, "could not parse argument to ext_wbuf_size\n");
                    return 1;
                }
            ...
            case EXT_THREADS:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_threads argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &ext_cf.io_threadcount)) {
                    my_fprintf(stderr, "could not parse argument to ext_threads\n");
                    return 1;
                }
                break;
            case EXT_IO_DEPTH:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_io_depth argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &ext_cf.io_depth)) {
                    my_fprintf(stderr, "could not parse argument to ext_io_depth\n");
                    return 1;
                }
                break;
            case EXT_ITEM_SIZE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_item_size argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_item_size)) {
                    my_fprintf(stderr, "could not parse argument to ext_item_size\n");
                    return 1;
                }
                break;
            case EXT_ITEM_AGE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_item_age argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_item_age)) {
                    my_fprintf(stderr, "could not parse argument to ext_item_age\n");
                    return 1;
                }
                break;
            case EXT_LOW_TTL:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_low_ttl argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_low_ttl)) {
                    my_fprintf(stderr, "could not parse argument to ext_low_ttl\n");
                    return 1;
                }
                break;
            case EXT_RECACHE_RATE:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_recache_rate argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_recache_rate)) {
                    my_fprintf(stderr, "could not parse argument to ext_recache_rate\n");
                    return 1;
                }
                break;
            case EXT_COMPACT_UNDER:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_compact_under argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_compact_under)) {
                    my_fprintf(stderr, "could not parse argument to ext_compact_under\n");
                    return 1;
                }
                break;
            case EXT_DROP_UNDER:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_drop_under argument\n");
                    return 1;
                }
                if (!safe_strtoul(subopts_value, &settings.ext_drop_under)) {
                    my_fprintf(stderr, "could not parse argument to ext_drop_under\n");
                    return 1;
                }
                break;
            case EXT_MAX_FRAG:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing ext_max_frag argument\n");
                    return 1;
                }
                if (!safe_strtod(subopts_value, &settings.ext_max_frag)) {
                    my_fprintf(stderr, "could not parse argument to ext_max_frag\n");
                    return 1;
                }
                break;
            case SLAB_AUTOMOVE_FREERATIO:
                if (subopts_value == NULL) {
                    my_fprintf(stderr, "Missing slab_automove_freeratio argument\n");
                    return 1;
                }
                if (!safe_strtod(subopts_value, &settings.slab_automove_freeratio)) {
                    my_fprintf(stderr, "could not parse argument to slab_automove_freeratio\n");
                    return 1;
                }
                break;
            ...
            case EXT_PATH:
                if (subopts_value) {
                    ...
                    if (tmp == NULL) {
                        my_fprintf(stderr, "failed to parse ext_path argument\n");
                        return 1;
                    }
                    ...
                    my_fprintf(stderr, "missing argument to ext_path, ie: ext_path=/d/file:5G\n");
                    return 1;
                }
                break;
#endif
            ...
            break;
        default:
            my_fprintf(stderr, "Illegal argument \"%c\"\n", c);
            return 1;
        }
    }

    if (settings.item_size_max < 1024) {
        my_fprintf(stderr, "Item max size cannot be less than 1024 bytes.\n");
        exit(EX_USAGE);
    }
    if (settings.item_size_max > (settings.maxbytes / 2)) {
        my_fprintf(stderr, "Cannot set item size limit higher than 1/2 of memory max.\n");
        exit(EX_USAGE);
    }
    if (settings.item_size_max > (1024 * 1024 * 1024)) {
        my_fprintf(stderr, "Cannot set item size limit higher than a gigabyte.\n");
        exit(EX_USAGE);
    }
    ...
    if (settings.slab_chunk_size_max > settings.item_size_max) {
        my_fprintf(stderr, "slab_chunk_max (bytes: %d) cannot be larger than -I (item_size_max %d)\n",
                settings.slab_chunk_size_max, settings.item_size_max);
        exit(EX_USAGE);
    }

    if (settings.item_size_max % settings.slab_chunk_size_max != 0) {
        my_fprintf(stderr, "-I (item_size_max: %d) must be evenly divisible by slab_chunk_max (bytes: %d)\n",
                settings.item_size_max, settings.slab_chunk_size_max);
        exit(EX_USAGE);
    }

    if (settings.slab_page_size % settings.slab_chunk_size_max != 0) {
        my_fprintf(stderr, "slab_chunk_max (bytes: %d) must divide evenly into %d (slab_page_size)\n",
                settings.slab_chunk_size_max, settings.slab_page_size);
        exit(EX_USAGE);
    }
#ifdef EXTSTORE
    if (storage_file) {
        if (settings.item_size_max > ext_cf.wbuf_size) {
            my_fprintf(stderr, "-I (item_size_max: %d) cannot be larger than ext_wbuf_size: %d\n",
                settings.item_size_max, ext_cf.wbuf_size);
            exit(EX_USAGE);
        }

        if (settings.udpport) {
            my_fprintf(stderr, "Cannot use UDP with extstore enabled (-U 0 to disable)\n");
            exit(EX_USAGE);
        }
    }
#endif
    ...
    if (settings.hot_lru_pct + settings.warm_lru_pct > 80) {
        my_fprintf(stderr, "hot_lru_pct + warm_lru_pct cannot be more than 80%% combined\n");
        exit(EX_USAGE);
    }

    if (settings.temp_lru && !start_lru_maintainer) {
        my_fprintf(stderr, "temporary_ttl requires lru_maintainer to be enabled\n");
        exit(EX_USAGE);
    }

    if (hash_init(hash_type) != 0) {
        my_fprintf(stderr, "Failed to initialize hash_algorithm!\n");
        exit(EX_USAGE);
    }
    ...
    if (settings.sasl) {
        if (!protocol_specified) {
            ...
            if (settings.binding_protocol != binary_prot) {
                my_fprintf(stderr, "ERROR: You cannot allow the ASCII protocol while using SASL.\n");
                exit(EX_USAGE);
            }
        }
    }
    ...
    if (settings.auth_file) {
        if (!protocol_specified) {
            ...
            if (settings.binding_protocol != ascii_prot) {
                my_fprintf(stderr, "ERROR: You cannot allow the BINARY protocol while using ascii authentication tokens.\n");
                exit(EX_USAGE);
            }
        }
    }
    ...
    if (settings.ssl_enabled) {
        if (!settings.port) {
            my_fprintf(stderr, "ERROR: You cannot enable SSL without a TCP port.\n");
            exit(EX_USAGE);
        }
    ...
    if (maxcore != 0) {
         ...
         */
        if (wrapper_getrlimit(RLIMIT_CORE, &rlim) == 0) {
            rlim_new.rlim_cur = rlim_new.rlim_max = RLIM_INFINITY;
            if (wrapper_setrlimit(RLIMIT_CORE, &rlim_new)!= 0) {
                /* failed. try raising just to the old max */
                rlim_new.rlim_cur = rlim_new.rlim_max = rlim.rlim_max;
                (void)wrapper_setrlimit(RLIMIT_CORE, &rlim_new);
            }
        }
        /*
        ...
        if ((wrapper_getrlimit(RLIMIT_CORE, &rlim) != 0) || rlim.rlim_cur == 0) {
            my_fprintf(stderr, "failed to ensure corefile creation\n");
            exit(EX_OSERR);
        }
    }
    ...
    if (wrapper_getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
        my_fprintf(stderr, "failed to getrlimit number of files\n");
        exit(EX_OSERR);
    } else {
        rlim.rlim_cur = settings.maxconns;
        rlim.rlim_max = settings.maxconns;
        if (wrapper_setrlimit(RLIMIT_NOFILE, &rlim) != 0) {
            my_fprintf(stderr, "failed to set rlimit for open files. Try starting as root or requesting smaller maxconns value.\n");
            exit(EX_OSERR);
        }
    }
    ...
    if (getuid() == 0 || geteuid() == 0) {
        if (username == 0 || *username == '\0') {
            my_fprintf(stderr, "can't run as root without the -u switch\n");
            exit(EX_USAGE);
        }
        if ((pw = getpwnam(username)) == 0) {
            my_fprintf(stderr, "can't find the user %s to switch to\n", username);
            exit(EX_NOUSER);
        }
        if (setgroups(0, NULL) < 0) {
            my_fprintf(stderr, "failed to drop supplementary groups\n");
            exit(EX_OSERR);
        }
        if (setgid(pw->pw_gid) < 0 || setuid(pw->pw_uid) < 0) {
            my_fprintf(stderr, "failed to assume identity of user %s\n", username);
            exit(EX_OSERR);
        }
    }
    ...
    if (do_daemonize) {
        ...
        if (daemonize(maxcore, settings.verbose) == -1) {
            my_fprintf(stderr, "failed to daemon() in order to daemonize\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (lock_memory) {
        ...
        if (res != 0) {
            my_fprintf(stderr, "warning: -k invalid, mlockall() failed: %s\n",
                    strerror(errno));
        }
#else
        my_fprintf(stderr, "warning: -k invalid, mlockall() not supported on this platform.  proceeding without.\n");
#endif
    }
	 ...
	 */
	mtcp_getconf(&mtcp_conf);
    mtcp_conf.num_cores = settings.num_threads;
	mtcp_setconf(&mtcp_conf);
    ...
    if (settings.auth_file) {
        if (settings.udpport) {
            my_fprintf(stderr, "Cannot use UDP with ascii authentication enabled (-U 0 to disable)\n");
            exit(EX_USAGE);
        }
        ...
        switch (authfile_load(settings.auth_file)) {
            ...
            case AUTHFILE_OPENFAIL:
                ...
                break;
            case AUTHFILE_OOM:
                my_fprintf(stderr, "Out of memory reading password file: %s", settings.auth_file);
                exit(EXIT_FAILURE);
                break;
            case AUTHFILE_MALFORMED:
                my_fprintf(stderr, "Authfile [%s] has a malformed entry. Should be 'user:password'", settings.auth_file);
                exit(EXIT_FAILURE);
                break;
            case AUTHFILE_OK:
                break;
    ...
    if (storage_file) {
        ...
        if (storage == NULL) {
            my_fprintf(stderr, "Failed to initialize external storage: %s\n",
                    extstore_err(eres));
    ...
    if (start_lru_crawler && start_item_crawler_thread() != 0) {
        my_fprintf(stderr, "Failed to enable LRU crawler thread\n");
        exit(EXIT_FAILURE);
    }
#ifdef EXTSTORE
    if (storage && start_storage_compact_thread(storage) != 0) {
        my_fprintf(stderr, "Failed to start storage compaction thread\n");
        exit(EXIT_FAILURE);
    }
    if (storage && start_storage_write_thread(storage) != 0) {
        my_fprintf(stderr, "Failed to start storage writer thread\n");
        exit(EXIT_FAILURE);
    }
...
#endif
        my_fprintf(stderr, "Failed to enable LRU maintainer thread\n");
        return 1;
    }
    ...
    if (stats_state.curr_conns + stats_state.reserved_fds >= settings.maxconns - 1) {
        my_fprintf(stderr, "Maxconns setting is too low, use -c to increase.\n");
        exit(EXIT_FAILURE);
    }
