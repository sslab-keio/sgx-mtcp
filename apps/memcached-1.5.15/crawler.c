...
#define wrapper_poll(fds, nfds, timeout) poll(fds, nfds, timeout)
#endif
...
#ifdef COMPILE_WITH_INTEL_SGX
static thread_mutex_t lru_crawler_lock = SGX_THREAD_MUTEX_INITIALIZER;
static thread_cond_t  lru_crawler_cond = SGX_THREAD_COND_INITIALIZER;
#else
static thread_mutex_t lru_crawler_lock = PTHREAD_MUTEX_INITIALIZER;
static thread_cond_t  lru_crawler_cond = PTHREAD_COND_INITIALIZER;
#endif
...
static int crawler_expired_init(crawler_module_t *cm, void *data) {
    ...
    if (data != NULL) {
        ...
        // init lock.
        thread_mutex_init(&d->lock, NULL);
        d->is_external = false;
        d->start_time = current_time;

        cm->data = d;
    }
    thread_mutex_lock(&d->lock);
...
static void crawler_expired_eval(crawler_module_t *cm, item *search, uint32_t hv, int i) {
    ...
    if ((search->exptime != 0 && search->exptime < current_time)
        ...
        if (settings.verbose > 1) {
            ...
            my_fprintf(stderr, "LRU crawler found an expired item (flags: %d, slab: %d): ",
...
static int lru_crawler_poll(crawler_client_t *c) {
    ...
    int ret = wrapper_poll(to_poll, 1, 1000);
...
static void *item_crawler_thread(void *arg) {
    ...
    thread_mutex_lock(&lru_crawler_lock);
    thread_cond_signal(&lru_crawler_cond);
    settings.lru_crawler = true;
    if (settings.verbose > 2)
        my_fprintf(stderr, "Starting LRU crawler background thread\n");
    while (do_run_lru_crawler_thread) {
    thread_cond_wait(&lru_crawler_cond, &lru_crawler_lock);
    ...
    while (crawler_count) {
        ...
        for (i = POWER_SMALLEST; i < LARGEST_ID; i++) {
            ...
            if (search == NULL ||
                ...
                if (settings.verbose > 2)
                    my_fprintf(stderr, "Nothing left to crawl for %d\n", i);
                lru_crawler_class_done(i);
                continue;
            }
            ...
            if (crawls_persleep-- <= 0 && settings.lru_crawler_sleep) {
                thread_mutex_unlock(&lru_crawler_lock);
    ...
    if (settings.verbose > 2)
        my_fprintf(stderr, "LRU crawler thread sleeping\n");
    ...
    thread_mutex_unlock(&lru_crawler_lock);
    if (settings.verbose > 2)
        my_fprintf(stderr, "LRU crawler thread stopping\n");

    return NULL;
}

static thread_t item_crawler_tid;
...
int stop_item_crawler_thread(void) {
    ...
    thread_cond_signal(&lru_crawler_cond);
    thread_mutex_unlock(&lru_crawler_lock);
    if ((ret = thread_join(item_crawler_tid, NULL)) != 0) {
        my_fprintf(stderr, "Failed to stop LRU crawler thread: %s\n", strerror(ret));
        return -1;
    }
...
int start_item_crawler_thread(void) {
    ...
    if ((ret = thread_create(&item_crawler_tid, NULL,
        item_crawler_thread, NULL)) != 0) {
        my_fprintf(stderr, "Can't create LRU crawler thread: %s\n",
            strerror(ret));
        thread_mutex_unlock(&lru_crawler_lock);
        return -1;
    }
...
static int do_lru_crawler_start(uint32_t id, uint32_t remaining) {
    ...
    thread_mutex_lock(&lru_locks[sid]);
    if (crawlers[sid].it_flags == 0) {
        if (settings.verbose > 2)
            my_fprintf(stderr, "Kicking LRU crawler off for LRU %u\n", sid);
    ...
    thread_mutex_unlock(&lru_locks[sid]);
    if (starts) {
        STATS_LOCK();
...
int init_lru_crawler(void *arg) {
    if (lru_crawler_initialized == 0) {
        ...
        storage = arg;
#endif
        if (thread_cond_init(&lru_crawler_cond, NULL) != 0) {
            my_fprintf(stderr, "Can't initialize lru crawler condition\n");
            return -1;
        }
        thread_mutex_init(&lru_crawler_lock, NULL);
