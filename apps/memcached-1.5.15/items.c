...
#ifdef COMPILE_WITH_INTEL_SGX
static thread_mutex_t lru_maintainer_lock = SGX_THREAD_MUTEX_INITIALIZER;
static thread_mutex_t cas_id_lock = SGX_THREAD_MUTEX_INITIALIZER;
static thread_mutex_t stats_sizes_lock = SGX_THREAD_MUTEX_INITIALIZER;
#else
static thread_mutex_t lru_maintainer_lock = PTHREAD_MUTEX_INITIALIZER;
static thread_mutex_t cas_id_lock = PTHREAD_MUTEX_INITIALIZER;
static thread_mutex_t stats_sizes_lock = PTHREAD_MUTEX_INITIALIZER;
#endif

void item_stats_reset(void) {
    int i;
    for (i = 0; i < LARGEST_ID; i++) {
        thread_mutex_lock(&lru_locks[i]);
        memset(&itemstats[i], 0, sizeof(itemstats_t));
        thread_mutex_unlock(&lru_locks[i]);
    }
}
...
#ifdef COMPILE_WITH_INTEL_SGX
static thread_mutex_t bump_buf_lock = SGX_THREAD_MUTEX_INITIALIZER;
#else
static thread_mutex_t bump_buf_lock = PTHREAD_MUTEX_INITIALIZER;
#endif
...
uint64_t get_cas_id(void) {
    ...
    thread_mutex_lock(&cas_id_lock);
    uint64_t next_id = ++cas_id;
    thread_mutex_unlock(&cas_id_lock);
    return next_id;
}
...
static unsigned int temp_lru_size(int slabs_clsid) {
    ...
    id |= TEMP_LRU;
    unsigned int ret;
    thread_mutex_lock(&lru_locks[id]);
    ret = sizes_bytes[id];
    thread_mutex_unlock(&lru_locks[id]);
    return ret;
}
...
static void item_link_q(item *it) {
    thread_mutex_lock(&lru_locks[it->slabs_clsid]);
    do_item_link_q(it);
    thread_mutex_unlock(&lru_locks[it->slabs_clsid]);
}
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c, const bool do_update) {
    ...
    if (it != NULL) {
        ...
        if (item_is_flushed(it)) {
            ...
            if (settings.verbose > 2) {
                my_fprintf(stderr, " -nuked by flush");
            }
            was_found = 2;
            ...
            thread_mutex_unlock(&c->thread->stats.mutex);
            if (settings.verbose > 2) {
                my_fprintf(stderr, " -nuked by expire");
            }
            was_found = 3;
        } else {
            if (do_update) {
...
static void lru_bump_buf_link_q(lru_bump_buf *b) {
    thread_mutex_lock(&bump_buf_lock);
    ...
    thread_mutex_unlock(&bump_buf_lock);
    return;
}
...
void *item_lru_bump_buf_create(void) {
    ...
    thread_mutex_init(&b->mutex, NULL);
...
static int lru_maintainer_juggle(const int slabs_clsid) {
    ...
    if (settings.lru_segmented) {
        thread_mutex_lock(&lru_locks[slabs_clsid|COLD_LRU]);
        if (tails[slabs_clsid|COLD_LRU]) {
            cold_age = current_time - tails[slabs_clsid|COLD_LRU]->time;
        }
        thread_mutex_unlock(&lru_locks[slabs_clsid|COLD_LRU]);
...
static void lru_maintainer_crawler_check(struct crawler_expired_data *cdata, logger *l) {
    ...
    for (i = POWER_SMALLEST; i < POWER_LARGEST; i++) {
        ...
        if (s->run_complete) {
            ...
            thread_mutex_unlock(&cdata->lock);
        }
...
#endif
static thread_t lru_maintainer_tid;
...
static void *lru_maintainer_thread(void *arg) {
    ...
    if (cdata == NULL) {
        my_fprintf(stderr, "Failed to allocate crawler data for LRU maintainer thread\n");
        abort();
    }
    thread_mutex_init(&cdata->lock, NULL);
    cdata->crawl_complete = true; // kick off the crawler.
    logger *l = logger_create();
    if (l == NULL) {
        my_fprintf(stderr, "Failed to allocate logger for LRU maintainer thread\n");
        abort();
    }
    ...
    thread_mutex_lock(&lru_maintainer_lock);
    if (settings.verbose > 2)
        my_fprintf(stderr, "Starting LRU maintainer background thread\n");
    while (do_run_lru_maintainer_thread) {
        thread_mutex_unlock(&lru_maintainer_lock);
        if (to_sleep)
            usleep(to_sleep);
        thread_mutex_lock(&lru_maintainer_lock);
    ...
    free(cdata);
    if (settings.verbose > 2)
        my_fprintf(stderr, "LRU maintainer thread stopping\n");
...
int stop_lru_maintainer_thread(void) {
    ...
    thread_mutex_unlock(&lru_maintainer_lock);
    if ((ret = thread_join(lru_maintainer_tid, NULL)) != 0) {
        my_fprintf(stderr, "Failed to stop LRU maintainer thread: %s\n", strerror(ret));
        return -1;
    }
...
int start_lru_maintainer_thread(void *arg) {
    ...
    if ((ret = thread_create(&lru_maintainer_tid, NULL,
        lru_maintainer_thread, arg)) != 0) {
        my_fprintf(stderr, "Can't create LRU maintainer thread: %s\n",
            strerror(ret));
        thread_mutex_unlock(&lru_maintainer_lock);
        return -1;
    }
...
int init_lru_maintainer(void) {
    if (lru_maintainer_initialized == 0) {
        thread_mutex_init(&lru_maintainer_lock, NULL);
