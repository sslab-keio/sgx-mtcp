 ...
 */
#ifdef COMPILE_WITH_INTEL_SGX
static thread_mutex_t slabs_lock = SGX_THREAD_MUTEX_INITIALIZER;
static thread_mutex_t slabs_rebalance_lock = SGX_THREAD_MUTEX_INITIALIZER;
#else
static thread_mutex_t slabs_lock = PTHREAD_MUTEX_INITIALIZER;
static thread_mutex_t slabs_rebalance_lock = PTHREAD_MUTEX_INITIALIZER;
#endif
...
static void * alloc_large_chunk_linux(const size_t limit)
{
    ...
    if (!pagesize) {
        my_fprintf(stderr, "Failed to get supported huge page size\n");
        return NULL;
    }

    if (settings.verbose > 1)
        my_fprintf(stderr, "huge page size: %zu\n", pagesize);

    /* This works because glibc simply uses mmap when the alignment is
     * above a certain limit. */
    ret = posix_memalign(&ptr, pagesize, limit);
    if (ret != 0) {
        my_fprintf(stderr, "Failed to get aligned memory chunk: %d\n", ret);
        return NULL;
    }

    ret = madvise(ptr, limit, MADV_HUGEPAGE);
    if (ret < 0) {
        my_fprintf(stderr, "Failed to set transparent hugepage hint: %d\n", ret);
        free(ptr);
        ptr = NULL;
    }
...
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes) {
    ...
    if (prealloc) {
...
#ifdef COMPILE_WITH_INTEL_SGX
        mem_base = untrusted_malloc(mem_limit);
#else
        ...
        if (mem_base != NULL) {
            ...
            my_fprintf(stderr, "Warning: Failed to allocate requested memory in"
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {
        ...
        if (slab_sizes == NULL)
            size *= factor;
        if (settings.verbose > 1) {
            my_fprintf(stderr, "slab class %3d: chunk size %9u perslab %7u\n",
...
static void slabs_preallocate (const unsigned int maxslabs) {
    ...
    for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
        ...
        if (do_slabs_newslab(i) == 0) {
            my_fprintf(stderr, "Error while preallocating slab memory!\n"
...
void fill_slab_stats_automove(slab_stats_automove *am) {
    ...
    thread_mutex_unlock(&slabs_lock);
}
...
void slabs_adjust_mem_requested(unsigned int id, size_t old, size_t ntotal)
{
    ...
    slabclass_t *p;
    if (id < POWER_SMALLEST || id > power_largest) {
        my_fprintf(stderr, "Internal error! Invalid slab class\n");
        abort();
    }
...
#ifdef COMPILE_WITH_INTEL_SGX
static thread_cond_t slab_rebalance_cond = SGX_THREAD_COND_INITIALIZER;
#else
static thread_cond_t slab_rebalance_cond = PTHREAD_COND_INITIALIZER;
#endif
...
static int slab_rebalance_start(void) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "Started a slab rebalance\n");
    }
                ...
                if ((hold_lock = item_trylock(hv)) == NULL) {
                    ...
                    if (refcount == 2) { /* item is linked but not busy */
                        ...
                        if (settings.verbose > 2) {
                            my_fprintf(stderr, "Slab reassign hit a busy item: refcount: %d (%d -> %d)\n",
...
static void slab_rebalance_finish(void) {
    ...
    if (settings.verbose > 1) {
        my_fprintf(stderr, "finished a slab move\n");
    }
}
...
static void *slab_rebalance_thread(void *arg) {
    ...
    while (do_run_slab_rebalance_thread) {
        ...
        if (slab_rebalance_signal == 0) {
            ...
            thread_cond_wait(&slab_rebalance_cond, &slabs_rebalance_lock);
        }
    }
    return NULL;
}
...
static enum reassign_result_type do_slabs_reassign(int src, int dst) {
    ...
    thread_cond_signal(&slab_rebalance_cond);
...
enum reassign_result_type slabs_reassign(int src, int dst) {
    ...
    if (thread_mutex_trylock(&slabs_rebalance_lock) != 0) {
        return REASSIGN_RUNNING;
    }
    ret = do_slabs_reassign(src, dst);
    thread_mutex_unlock(&slabs_rebalance_lock);
    return ret;
}

/* If we hold this lock, rebalancer can't wake up or move */
void slabs_rebalancer_pause(void) {
    thread_mutex_lock(&slabs_rebalance_lock);
}
...
int start_slab_maintenance_thread(void) {
    ...
    if (thread_cond_init(&slab_rebalance_cond, NULL) != 0) {
        my_fprintf(stderr, "Can't initialize rebalance condition\n");
        return -1;
    }
    thread_mutex_init(&slabs_rebalance_lock, NULL);

    if ((ret = thread_create(&rebalance_tid, NULL,
                              slab_rebalance_thread, NULL)) != 0) {
        my_fprintf(stderr, "Can't create rebal thread: %s\n", strerror(ret));
        return -1;
    }
    return 0;
}
...
void stop_slab_maintenance_thread(void) {
    ...
    thread_join(rebalance_tid, NULL);
}
