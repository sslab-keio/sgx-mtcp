...
/*----------------------------------------------------------------------------*/
#ifndef DISABLE_DEBUGGING_AND_LOGGING
static void
EnqueueFreeBuffer(log_thread_context *ctx, log_buff *free_bp) 
{
	thread_mutex_lock(&ctx->free_mutex);
	...
	thread_mutex_unlock(&ctx->free_mutex);
}
/*----------------------------------------------------------------------------*/
log_buff*
...
InitLogThreadContext(struct log_thread_context *ctx, int cpu) 
{
	...
	if (pipe(sv)) {
		my_fprintf(stderr, "pipe() failed, errno=%d, errstr=%s\n",
	...
	thread_mutex_init(&ctx->mutex, NULL);
	thread_mutex_init(&ctx->free_mutex, NULL);
	...
	log_buff *w_buff = untrusted_malloc(sizeof(log_buff) * NUM_LOG_BUFF);
#else
...
ThreadLogMain(void* arg)
{
	...
	//my_fprintf(stderr, "[CPU %d] Log thread created. thread: %lu\n",
	//		ctx->cpu, thread_self());
