...
PrintTCPOptions(uint8_t *tcpopt, int len)
{
	...
	for (i = 0; i < len; ) {
		...
		if (opt == TCP_OPT_END) {	// end of option field
			...
			my_printf("Option: %d", opt);
			my_printf(", length: %d", optlen);
			...
			if (opt == TCP_OPT_MSS) {
				...
				i += 4;
				my_printf(", TSval: %u, TSref: %u", ts_val, ts_ref);
			} else if (opt == TCP_OPT_WSCALE) {
				uint8_t wscale;
				wscale = *(tcpopt + i++);
				my_printf(", Wscale: %u", wscale);
			} else {
				// not handle
				i += optlen - 2;
			}
