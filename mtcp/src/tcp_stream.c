...
RaiseCloseEvent(mtcp_manager_t mtcp, tcp_stream *stream)
{
	if (stream->socket) {
		if (stream->socket->epoll & MTCP_EPOLLRDHUP) {
			...
			//thread_cond_signal(&stream->rcvvar->read_cond);
			//thread_cond_signal(&stream->sndvar->write_cond);
			if (!stream->on_rcv_br_list) {
				stream->on_rcv_br_list = TRUE;
				TAILQ_INSERT_TAIL(&mtcp->rcv_br_list, stream, rcvvar->rcv_br_link);
				mtcp->rcv_br_list_cnt++;
		...
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
	...
	thread_mutex_lock(&mtcp->ctx->flow_pool_lock);
	...
	if (!stream) {
		...
		thread_mutex_unlock(&mtcp->ctx->flow_pool_lock);
		return NULL;
	}
    ...
    stream->sndvar->iss = val % TCP_MAX_SEQ;
#else
...
#if BLOCKING_SUPPORT
	if (thread_cond_init(&stream->rcvvar->read_cond, NULL)) {
		perror("thread_cond_init of read_cond");
		return NULL;
	}
	if (thread_cond_init(&stream->sndvar->write_cond, NULL)) {
		perror("thread_cond_init of write_cond");
		return NULL;
	}
#endif

#if USE_SPIN_LOCK
	if (thread_spin_init(&stream->rcvvar->read_lock, PTHREAD_PROCESS_PRIVATE)) {
#else
	if (thread_mutex_init(&stream->rcvvar->read_lock, NULL)) {
#endif
		perror("thread_mutex_init of read_lock");
#if BLOCKING_SUPPORT
		thread_cond_destroy(&stream->rcvvar->read_cond);
		thread_cond_destroy(&stream->sndvar->write_cond);
#endif
		return NULL;
	}
#if USE_SPIN_LOCK
	if (thread_spin_init(&stream->sndvar->write_lock, PTHREAD_PROCESS_PRIVATE)) {
		perror("thread_spin_init of write_lock");
		thread_spin_destroy(&stream->rcvvar->read_lock);
#else
	if (thread_mutex_init(&stream->sndvar->write_lock, NULL)) {
		perror("thread_mutex_init of write_lock");
		thread_mutex_destroy(&stream->rcvvar->read_lock);
#endif
#if BLOCKING_SUPPORT
		thread_cond_destroy(&stream->rcvvar->read_cond);
		thread_cond_destroy(&stream->sndvar->write_cond);
...
DestroyTCPStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
	...
	if (thread_cond_destroy(&stream->rcvvar->read_cond)) {
		perror("thread_cond_destroy of read_cond");
	}
	if (thread_cond_destroy(&stream->sndvar->write_cond)) {
		perror("thread_cond_destroy of write_cond");
	}
#endif
