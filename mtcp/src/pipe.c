...
struct pipe
{
	...
	thread_mutex_t pipe_lock;
	thread_cond_t pipe_cond;
};
/*---------------------------------------------------------------------------*/
int 
mtcp_pipe(mctx_t mctx, int pipeid[2])
{
	...
	ret = thread_mutex_init(&pp->pipe_lock, NULL);
	if (ret) {
		/* errno set by thread_mutex_init() */
		FreeSocket(mctx, socket[0]->id, FALSE);
		FreeSocket(mctx, socket[1]->id, FALSE);
		free(pp->buf);
		free(pp);
	...
	ret = thread_cond_init(&pp->pipe_cond, NULL);
	if (ret) {
		/* errno set by thread_cond_init() */
		FreeSocket(mctx, socket[0]->id, FALSE);
		FreeSocket(mctx, socket[1]->id, FALSE);
		free(pp->buf);
		thread_mutex_destroy(&pp->pipe_lock);
		free(pp);
		return -1;
	}
...
RaiseEventToPair(mtcp_manager_t mtcp, socket_map_t socket, uint32_t event)
{
	...
	if (pair_socket->opts & MTCP_NONBLOCK) {
		...
		thread_cond_signal(&pp->pipe_cond);
	}
}
/*---------------------------------------------------------------------------*/
int 
PipeRead(mctx_t mctx, int pipeid, char *buf, int len)
{
	...
	thread_mutex_lock(&pp->pipe_lock);
	if (!(socket->opts & MTCP_NONBLOCK)) {
		while (pp->buf_len == 0) {
			ret = thread_cond_wait(&pp->pipe_cond, &pp->pipe_lock);
			if (ret) {
				/* errno set by thread_cond_wait() */
				thread_mutex_unlock(&pp->pipe_lock);
				return -1;
			}
		}
	}
...
PipeClose(mctx_t mctx, int pipeid)
{
	...
	thread_mutex_destroy(&pp->pipe_lock);
	thread_cond_destroy(&pp->pipe_cond);
