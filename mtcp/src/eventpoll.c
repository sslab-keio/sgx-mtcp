...
/*----------------------------------------------------------------------------*/
int
ecall_mtcp_epoll_create(mctx_t mctx, int size) {
...
mtcp_epoll_create(mctx_t mctx, int size)
{
	...
	if (thread_mutex_init(&ep->epoll_lock, NULL)) {
	...
	if (thread_cond_init(&ep->epoll_cond, NULL)) {
		DestroyEventQueue(ep->mtcp_queue);
		DestroyEventQueue(ep->usr_shadow_queue);
		DestroyEventQueue(ep->usr_queue);
		FreeSocket(mctx, epsocket->id, FALSE);
...
CloseEpollSocket(mctx_t mctx, int epid)
{
	...
	thread_cond_destroy(&ep->epoll_cond);
	thread_mutex_destroy(&ep->epoll_lock);
	free(ep);
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_epoll_ctl(mctx_t mctx, int epid, int op, int sockid, struct mtcp_epoll_event *event) {
    return mtcp_epoll_ctl(mctx, epid, op, sockid, event);
}
int ecall_mtcp_batch_epoll_ctl(mctx_t mctx, int epid, int op, int *sockids, int sockids_len, struct mtcp_epoll_event *event) {
  int i, ret=0;
  for (i=0; i<sockids_len; i++) {
    if (event != NULL)
      event->data.sockid = sockids[i];
    ret = mtcp_epoll_ctl(mctx, epid, op, sockids[i], event);
    if (ret < 0)
      return ret;
  }
  return 0;
...
int mtcp_batch_epoll_ctl(mctx_t mctx, int epid, int op, int *sockids, int sockids_len, struct mtcp_epoll_event *event) {
  return -1;
}
/*----------------------------------------------------------------------------*/
int
ecall_mtcp_epoll_wait(mctx_t mctx, int epid,
        struct mtcp_epoll_event *events, int maxevents, int timeout) {
    return mtcp_epoll_wait(mctx, epid, events, maxevents, timeout);
}
int
		...
		struct mtcp_epoll_event *events, int maxevents, int timeout)
{
	...
	int spin = 0;
	while (ep->usr_queue->num_events == 0 && ep->usr_shadow_queue->num_events == 0 && spin < SPIN_THRESH) {
		spin++;
	}
#endif /* SPIN_BEFORE_SLEEP */

#if !ENABLE_UCTX
	if (thread_mutex_lock(&ep->epoll_lock)) {
		if (errno == EDEADLK)
...
wait:
	...
	while (eq->num_events == 0 && eq_shadow->num_events == 0 && timeout != 0) {
...
#else
			thread_kill(mtcp->ctx->thread, SIGUSR1);
#endif
		}
#endif
		ep->stat.waits++;
...
#else
			ret = thread_cond_timedwait(&ep->epoll_cond,
					&ep->epoll_lock, &deadline);
			if (ret && ret != ETIMEDOUT) {
				/* errno set by thread_cond_timedwait() */
				thread_mutex_unlock(&ep->epoll_lock);
				TRACE_ERROR("thread_cond_timedwait failed. ret: %d, error: %s\n",
						ret, strerror(errno));
				return -1;
			}
#endif
      ...
      while (ep->waiting)
				YieldToStack(mtcp->ctx, YIELD_REASON_EPOLL);
#else
			ret = thread_cond_wait(&ep->epoll_cond, &ep->epoll_lock);
			if (ret) {
				/* errno set by thread_cond_wait() */
				thread_mutex_unlock(&ep->epoll_lock);
				TRACE_ERROR("thread_cond_wait failed. ret: %d, error: %s\n",
						ret, strerror(errno));
				return -1;
			}
#endif
...
#if !ENABLE_UCTX
			//ret = thread_cond_signal(&ep->epoll_cond);
			thread_mutex_unlock(&ep->epoll_lock);
#endif
			errno = EINTR;
			return -1;
...
#if !COMPILE_WITH_INTEL_SGX || !ENABLE_UCTX
	thread_mutex_unlock(&ep->epoll_lock);
#endif
