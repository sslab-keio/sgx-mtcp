...
#define TRACE_TEMP(f, m...) { \
	my_fprintf(stderr, "[CPU %d][%10s:%4d] " f, mtcp->ctx->cpu, 		\
...
#define TRACE_ERROR(f, m...) { \
	my_fprintf(stderr, "[%10s:%4d] " f, __FUNCTION__, __LINE__, ##m);	\
	}
...
#define CTRACE_ERROR(f, m...) { \
	my_fprintf(stderr, "[CPU %d][%10s:%4d] " f, mtcp->ctx->cpu, __FUNCTION__, __LINE__, ##m);	\
	}
...
#define TRACE_CONFIG(f, m...) my_fprintf(stderr, f, ##m)
