...
#define SBUF_LOCK_INIT(lock, errmsg, action);		\
	if (thread_spin_init(lock, PTHREAD_PROCESS_PRIVATE)) {		\
		perror("thread_spin_init" errmsg);			\
		action;										\
	}
#define SBUF_LOCK_DESTROY(lock)	thread_spin_destroy(lock)
#define SBUF_LOCK(lock)			thread_spin_lock(lock)
#define SBUF_UNLOCK(lock)		thread_spin_unlock(lock)
#else
#define SBUF_LOCK_INIT(lock, errmsg, action);		\
	if (thread_mutex_init(lock, NULL)) {			\
		perror("thread_mutex_init" errmsg);		\
		action;										\
	}
#define SBUF_LOCK_DESTROY(lock)	thread_mutex_destroy(lock)
#define SBUF_LOCK(lock)			thread_mutex_lock(lock)
#define SBUF_UNLOCK(lock)		thread_mutex_unlock(lock)
#endif /* USE_SPIN_LOCK */
...
struct mtcp_thread_context
{
	...
	thread_mutex_t smap_lock;
	thread_mutex_t flow_pool_lock;
	thread_mutex_t socket_pool_lock;

#if LOCK_STREAM_QUEUE
#if USE_SPIN_LOCK
	thread_spinlock_t connect_lock;
	thread_spinlock_t close_lock;
	thread_spinlock_t reset_lock;
	thread_spinlock_t sendq_lock;
	thread_spinlock_t ackq_lock;
	thread_spinlock_t destroyq_lock;
#else
	thread_mutex_t connect_lock;
	thread_mutex_t close_lock;
	thread_mutex_t reset_lock;
	thread_mutex_t sendq_lock;
	thread_mutex_t ackq_lock;
	thread_mutex_t destroyq_lock;
#endif /* USE_SPIN_LOCK */
