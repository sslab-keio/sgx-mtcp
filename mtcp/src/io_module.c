...
#define ioctl1(fd, request, size, ...) ioctl(fd, request, ##__VA_ARGS__)
#endif
...
probe_all_rte_devices(char **argv, int *argc, char *dev_name_list)
{
    ...
    len = strlen(dev_name_list);
	dev_tokenizer= strndup(dev_name_list, len);
    ...
    // XXX: Somehow, strcpy is not working correctly w/ sgx
    /* strcpy(pd.ifname, dev_token); */
    strncpy(pd.ifname, dev_token, strlen(dev_token) + 1);
    if (ioctl1(fd, FETCH_PCI_ADDRESS, sizeof(pd), &pd) == -1) {
    ...
    // For bluefield
    if (strcmp(pd.ifname, "enp1s0f0") == 0) {
      my_printf("[DEBUG] %s is Mellanox Bluefield NIC\n", pd.ifname);
      my_printf("[DEBUG] PCI adress of %s is %s\n", pd.ifname, "0000:01:00.0");
      sprintf(argv[*argc + 1], "0000:01:00.0");
      *argc += 2;
    } else {
...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
	...
	if (current_iomodule_func == &ps_module_func) {
		...
		for (i = 0; i < num_devices; i++) {
			...
			if (ioctl(sock, SIOCGIFFLAGS, sizeof(ifr), &ifr) == 0) {
				...
				/* Net MASK */
				if (ioctl(sock, SIOCGIFNETMASK, sizeof(ifr), &ifr) == 0) {
					struct in_addr sin = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
		...
		if (_cpumask == CONFIG._cpumask) {
			/* get the cpu mask */
			for (ret = 0; ret < cpu; ret++)
                _cpumask |= 1UL << ret;

            sprintf(cpumaskbuf, "%zx", _cpumask);
		} else
			sprintf(cpumaskbuf, "%zx", CONFIG._cpumask);
		...
		do {
			if (iter_if->ifa_addr && iter_if->ifa_addr->sa_family == AF_INET &&
			    ...
			    *(seek + strlen(iter_if->ifa_name)) != ':') {
				...
				/* getting address */
				if (ioctl1(sock, SIOCGIFADDR, sizeof(ifr), &ifr) == 0 ) {
					struct in_addr sin = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
					CONFIG.eths[eidx].ip_addr = *(uint32_t *)&sin;
				}

				if (ioctl1(sock, SIOCGIFHWADDR, sizeof(ifr), &ifr) == 0 ) {
					for (j = 0; j < ETH_ALEN; j ++) {
						CONFIG.eths[eidx].haddr[j] = ifr.ifr_addr.sa_data[j];
					}
				}
				...
				/* Net MASK */
				if (ioctl1(sock, SIOCGIFNETMASK, sizeof(ifr), &ifr) == 0) {
					struct in_addr sin = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
					CONFIG.eths[eidx].netmask = *(uint32_t *)&sin;
				}
				close(sock);
				...
				num_devices_attached++;
				my_fprintf(stderr, "Total number of attached devices: %d\n",
					num_devices_attached);
				my_fprintf(stderr, "Interface name: %s\n",
					iter_if->ifa_name);
			}
		...
		for (; rte_argc >= 9; rte_argc--) {
			if (rte_argv[rte_argc] != NULL) {
				my_fprintf(stderr, "Cleaning up rte_argv[%d]: %s (%p)\n",
