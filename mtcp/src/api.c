...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_getsockopt(mctx_t mctx, int sockid, int level, int optname, void *optval, socklen_t *optlen) {
  return mtcp_getsockopt(mctx, sockid, level, optname, optval, optlen);
}
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_setsock_nonblock(mctx_t mctx, int sockid) {
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_socket_ioctl(mctx_t mctx, int sockid, int request, void *argp) {
    return mtcp_socket_ioctl(mctx, sockid, request, argp);
}
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_socket(mctx_t mctx, int domain, int type, int protocol){
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_bind(mctx_t mctx, int sockid, const struct sockaddr *addr, socklen_t addrlen) {
  return mtcp_bind(mctx, sockid, addr, addrlen);
}
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_listen(mctx_t mctx, int sockid, int backlog) {
  return mtcp_listen(mctx, sockid, backlog);
}
...
mtcp_listen(mctx_t mctx, int sockid, int backlog)
{
	...
	if (thread_cond_init(&listener->accept_cond, NULL)) {
		/* errno set internally */
		perror("thread_cond_init of ctx->accept_cond\n");
		free(listener);
		return -1;
	}
	if (thread_mutex_init(&listener->accept_lock, NULL)) {
		/* errno set internally */
		perror("thread_mutex_init of ctx->accept_lock\n");
		free(listener);
		return -1;
	}
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen, int* out_errno_ref) {
  int ret = mtcp_accept(mctx, sockid, addr, addrlen);
  if (ret == -1) {
	  *out_errno_ref = errno;
  }
  return ret;
...
int ecall_mtcp_batch_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen, int *sockids, int maxaccepts, int* out_errno_ref) {
  int ret = mtcp_batch_accept(mctx, sockid, addr, addrlen, sockids, maxaccepts);
  if (ret == -1) {
	  *out_errno_ref = errno;
  }
  return ret;
...
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen)
{
      ...
      while ((accepted = StreamDequeue(listener->acceptq)) == NULL) {
        ...
        if (mtcp->ctx->done || mtcp->ctx->exit) {
          ...
          return -1;
			...
			while ((accepted = StreamDequeue(listener->acceptq)) == NULL) {
				thread_cond_wait(&listener->accept_cond, &listener->accept_lock);
	...
	accepted->socket->opts |= MTCP_NONBLOCK;

	return accepted->socket->id;
}

int mtcp_batch_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen, int *sockids, int maxaccepts) {
  int i;
  int ret=0;

  for (i=0; i<maxaccepts; i++) {
    ret = mtcp_accept(mctx, sockid, addr, addrlen);
    sockids[i] = ret;
    if (ret == -1) {
      if (errno == EAGAIN) {
        if (i != 0) { // Return num of created connections
          return i;
        }
      }
      return ret;
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_init_rss(mctx_t mctx, in_addr_t saddr_base, int num_addr, in_addr_t daddr, in_addr_t dport) {
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_connect(mctx_t mctx, int sockid, const struct sockaddr *addr, socklen_t addrlen) {
...
CloseListeningSocket(mctx_t mctx, int sockid)
{
	...
	thread_cond_destroy(&listener->accept_cond);
	thread_mutex_destroy(&listener->accept_lock);
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_close(mctx_t mctx, int sockid) {
  return mtcp_close(mctx, sockid);
}

int ecall_mtcp_batch_close(mctx_t mctx, int *sockids, int sockids_len) {
  int i;
  int ret=0;
  for (i=0; i<sockids_len; i++) {

    ret = mtcp_close(mctx, sockids[i]);
    if (ret < 0)
      return ret;
  }
  return ret;
...
// This function shouldn't be used without sgx
int mtcp_batch_close(mctx_t mctx, int *sockids, int sockids_len) {
  return -1;
}
/*----------------------------------------------------------------------------*/
int
...
/*----------------------------------------------------------------------------*/
ssize_t
ecall_mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags, int* out_errno_ref) {
    ssize_t ret = mtcp_recv(mctx, sockid, buf, len, flags);
	 if (ret == -1) {
		 *out_errno_ref = errno;
	 }
	 return ret;
...
mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags)
{
	...
	if (!(socket->opts & MTCP_NONBLOCK)) {
		while (rcvvar->rcvbuf->merged_len == 0) {
			...
			thread_cond_wait(&rcvvar->read_cond, &rcvvar->read_lock);
		}
	}
#endif
...
/*----------------------------------------------------------------------------*/
ssize_t ecall_mtcp_write(mctx_t mctx, int sockid, const char *buf, size_t len, int* out_errno_ref) {
  ssize_t ret = mtcp_write(mctx, sockid, buf, len);
  if (ret == -1) {
    *out_errno_ref = errno;
  }
  return ret;
...
mtcp_write(mctx_t mctx, int sockid, const char *buf, size_t len)
{
	...
	if (!(socket->opts & MTCP_NONBLOCK)) {
		while (sndvar->snd_wnd <= 0) {
			...
			thread_cond_wait(&sndvar->write_cond, &sndvar->write_lock);
...
/*----------------------------------------------------------------------------*/
int
ecall_mtcp_writev(mctx_t mctx, int sockid, const struct iovec *iov, int numIOV) {
	return mtcp_writev(mctx, sockid, iov, numIOV);
}
int
