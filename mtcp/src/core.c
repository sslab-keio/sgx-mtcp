...
#define wrapper_rte_get_master_lcore() rte_get_master_lcore()
#endif
#endif
...
/*----------------------------------------------------------------------------*/
#ifdef ENABLE_UCTX
static struct lthread * g_thread[MAX_CPUS] = {0};
static struct lthread * g_app_thread[MAX_CPUS] = {0};
#else
static thread_t g_thread[MAX_CPUS] = {0};
#endif
#if defined (PKTDUMP) || (DBGMSG) || (DBGFUNC) ||		\
	(STREAM) || (STATE) || (STAT) || (APP) || (EPOLL)	\
	|| (DUMP_STREAM)
static thread_t log_thread[MAX_CPUS]  = {0};
#endif
/*----------------------------------------------------------------------------*/
static sem_t* g_init_sem = NULL;
...
PrintThreadNetworkStats(mtcp_manager_t mtcp, struct net_stat *ns)
{
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		...
		if (CONFIG.eths[i].stat_print) {
			my_fprintf(stderr, "[CPU%2d] %s flows: %6u, "
...
#if NETSTAT_PERTHREAD
	my_fprintf(stderr, "[CPU%2d] Rounds: %4ldK, "
...
{
	my_fprintf(stderr, "[CPU%2d] calls: %lu, waits: %lu, wakes: %lu, "
...
#define TIMEOUT 1
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		if (CONFIG.eths[i].stat_print) {
			my_fprintf(stderr, "[ ALL ] %s flows: %6u, "
					"RX: %7ld(pps) (err: %5ld), %5.2lf(Gbps), "
...
#ifdef ENABLELRO
	my_fprintf(stderr, "[ ALL ] Goodput RX: %5.2lf(Gbps), TX: %5.2lf(Gbps)\n",
...
#if 0
	my_fprintf(stderr, "[ ALL ] Rounds: %4ldK, "
			"rx: %3ldK (try: %4ldK), tx: %3ldK (try: %4ldK), "
...
FlushAcceptEvents(mtcp_manager_t mtcp)
{
	...
	thread_mutex_lock(&mtcp->listener->accept_lock);
	if (!StreamQueueIsEmpty(mtcp->listener->acceptq)) {
		thread_cond_signal(&mtcp->listener->accept_cond);
	}
	thread_mutex_unlock(&mtcp->listener->accept_lock);
}
/*----------------------------------------------------------------------------*/
static inline void
	...
	while (walk) {
		...
		if (walk->on_snd_br_list) {
			...
			if (!(walk->epoll & MTCP_EPOLLOUT)) {
				thread_cond_signal(&walk->write_cond);
	...
	while (walk) {
		...
		if (walk->on_rcv_br_list) {
			if (!(walk->epoll & MTCP_EPOLLIN)) {
				...
				thread_cond_signal(&walk->read_cond);
...
FlushEpollEvents(mtcp_manager_t mtcp, uint32_t cur_ts)
{
...
#else
	thread_mutex_lock(&ep->epoll_lock);
	if (ep->mtcp_queue->num_events > 0) {
		/* while mtcp_queue have events */
		/* and usr_queue is not full */
		while (mtcpq->num_events > 0 && usrq->num_events < usrq->size) {
	...
	if (ep->waiting && (ep->usr_queue->num_events > 0 || 
		...
		mtcp->ts_last_event = cur_ts;
		ep->stat.wakes++;
		thread_cond_signal(&ep->epoll_cond);
	}
	thread_mutex_unlock(&ep->epoll_lock);
#endif
}
/*----------------------------------------------------------------------------*/
static inline void 
...
InterruptApplication(mtcp_manager_t mtcp)
{
	...
	/* interrupt if the mtcp_epoll_wait() is waiting */
	if (mtcp->ep) {
		thread_mutex_lock(&mtcp->ep->epoll_lock);
		if (mtcp->ep->waiting) {
			thread_cond_signal(&mtcp->ep->epoll_cond);
		}
		thread_mutex_unlock(&mtcp->ep->epoll_lock);
	}
#endif
	...
	for (i = 0; i < MAX_PORT; i++) {
		listener = ListenerHTSearch(mtcp->listeners, &i);
		if (listener != NULL) {
			thread_mutex_lock(&listener->accept_lock);
			if (!(listener->socket->opts & MTCP_NONBLOCK)) {
				thread_cond_signal(&listener->accept_cond);
			}
			thread_mutex_unlock(&listener->accept_lock);
		}
	}
#endif
}
...
#ifdef COMPILE_WITH_INTEL_SGX
  assert(sgx_is_within_enclave(&cur_ts, sizeof(struct timeval)) == 1);
#endif

	TRACE_DBG("CPU %d: mtcp thread running.\n", ctx->cpu);

  gettimeofday(&cur_ts, NULL);
	ts = ts_prev = TIMEVAL_TO_TS(&cur_ts);
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
		...
		for (rx_tx_inf = 0; rx_tx_inf < CONFIG.eths_num; rx_tx_inf++) {
			static uint16_t len;
			static uint8_t *pktbuf;
			recv_cnt = mtcp->iom->send_pkts_and_recv_pkts_and_gettimeofday(ctx, rx_tx_inf, &cur_ts);
			if (rx_tx_inf == 0) {
			...
			for (i = 0; i < recv_cnt; i++) {
				pktbuf = mtcp->iom->get_rptr(mtcp->ctx, rx_tx_inf, i, &len);
				if (pktbuf != NULL) {
					ProcessPacket(mtcp, rx_tx_inf, ts, pktbuf, len);
				}
#ifdef NETSTAT
				else
					mtcp->nstat.rx_errors[rx_tx_inf]++;
#endif
			}
		}
    ...
    /* Comment out send_pkts
     * because send_pkts and recv_pkts are merged
     */
...
InitializeMTCPManager(struct mtcp_thread_context* ctx)
{
	...
	if (!mtcp) {
		perror("malloc");
		my_fprintf(stderr, "Failed to allocate mtcp_manager.\n");
		return NULL;
	}
	g_mtcp[ctx->cpu] = mtcp;
...
#else
	ctx->thread = thread_self();
#endif

#if !ENABLE_UCTX
	/* affinitize the thread to this core first */
#ifndef DISABLE_DPDK
	if (wrapper_rte_lcore_id() == LCORE_ID_ANY)
#endif
	{
		mtcp_core_affinitize(cpu);
	}

	thread_sem_post(&g_init_sem[ctx->cpu]);
#endif
...
mctx_t ecall_mtcp_create_context(int cpu) {
    return mtcp_create_context(cpu);
}
mctx_t 
mtcp_create_context(int cpu)
{
...
#if !ENABLE_UCTX
	int ret;
	ret = thread_sem_init(&g_init_sem[cpu], 0, 0);
	if (ret) {
	...
	(EPOLL) || (DUMP_STREAM)
	if (thread_create(&log_thread[cpu], 
	...
	if (thread_mutex_init(&ctx->smap_lock, NULL)) {
		perror("thread_mutex_init of ctx->smap_lock\n");
		exit(-1);
	}

	if (thread_mutex_init(&ctx->flow_pool_lock, NULL)) {
		perror("thread_mutex_init of ctx->flow_pool_lock\n");
		exit(-1);
	}
	
	if (thread_mutex_init(&ctx->socket_pool_lock, NULL)) {
		perror("thread_mutex_init of ctx->socket_pool_lock\n");
		exit(-1);
	}
	...
	my_fprintf(stderr, "CPU %d: initialization finished.\n", cpu);

#ifndef DISABLE_DPDK
	/* Wake up mTCP threads (wake up I/O threads) */
	if (current_iomodule_func == &dpdk_module_func) {
		int master;
		master = wrapper_rte_get_master_lcore();
		
		if (master == whichCoreID(cpu)) {
#ifdef COMPILE_WITH_INTEL_SGX
		    set_lcore_config(master, 0, FINISHED);
#else
...
#ifdef ENABLE_UCTX
			if (lthread_create(&g_thread[cpu], cpu,
					   (lthread_func_t)MTCPRunThread, (void *)mctx) != 0) {
#else
			if (thread_create(&g_thread[cpu], 
					   NULL, MTCPRunThread, (void *)mctx) != 0) {
#endif
				TRACE_ERROR("thread_create of mtcp thread failed!\n");
				return NULL;
			}
		} else {
#ifdef COMPILE_WITH_INTEL_SGX
      set_lcore_config(whichCoreID(cpu), 0, FINISHED);
#else
			lcore_config[whichCoreID(cpu)].ret = 0;
			lcore_config[whichCoreID(cpu)].state = FINISHED;
#endif
#ifdef ENABLE_UCTX
			if (lthread_create(&g_thread[cpu], cpu,
					   (lthread_func_t)MTCPRunThread, (void *)mctx) != 0) {
...
#if !ENABLE_UCTX
	thread_sem_wait(&g_init_sem[cpu]);
	thread_sem_destroy(&g_init_sem[cpu]);
#endif
...
/*----------------------------------------------------------------------------*/
void
ecall_mtcp_destroy_context(mctx_t mctx) {
...
mtcp_free_context(mctx_t mctx)
{
	...
	//thread_kill(g_thread[mctx->cpu], SIGINT);
	...
	thread_join(log_thread[ctx->cpu], NULL);
#endif
	fclose(mtcp->log_fp);
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_getconf(struct mtcp_conf *conf) {
    return mtcp_getconf(conf);
}
int 
...
/*----------------------------------------------------------------------------*/
int ecall_mtcp_setconf(const struct mtcp_conf *conf) {
    return mtcp_setconf(conf);
}
int 
...
/*----------------------------------------------------------------------------*/
int
ecall_mtcp_init(const char *config_file) {
...
#ifdef COMPILE_WITH_INTEL_SGX
	g_init_sem = untrusted_calloc(MAX_CPUS, sizeof(*g_init_sem));
#else
	g_init_sem = calloc(MAX_CPUS, sizeof(*g_init_sem));
#endif
...
mtcp_destroy()
{
	int i;
#ifndef DISABLE_DPDK
	int master = wrapper_rte_get_master_lcore();
#endif
...
#ifdef ENABLE_UCTX
#ifdef COMPILE_WITH_INTEL_SGX
				lthread_join(g_thread[i], NULL, 0);
#else
				lthread_join(g_thread[i], NULL);
#endif
#else
				thread_join(g_thread[i], NULL);
#endif
			}
		}
	}
