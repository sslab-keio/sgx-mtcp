...
#ifdef COMPILE_WITH_INTEL_SGX
static int dev_info_initialized[RTE_MAX_ETHPORTS] = {0};
#endif
...
/*----------------------------------------------------------------------------*/
int
dpdk_prepare_send_merged_pkts(struct mtcp_thread_context *ctxt, int ifidx, int num_of_queues)
{
  return 0;
}
/*----------------------------------------------------------------------------*/
int
dpdk_send_merged_pkts(struct mtcp_thread_context *ctxt, int ifidx)
{
  return -1;
}
/*----------------------------------------------------------------------------*/
...
{
#ifdef COMPILE_WITH_INTEL_SGX
    return sgx_dpdk_send_pkts(ctxt, ifidx);
#else
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	mtcp_manager_t mtcp;
...
{
#ifdef COMPILE_WITH_INTEL_SGX
    return sgx_dpdk_recv_pkts(ctxt, ifidx);
#else
	struct dpdk_private_context *dpc;
	int ret;
...
#if 0
  for (int i=0; i<ret; i++) {
    rte_pktmbuf_dump(stdout, dpc->pkts_burst[i], dpc->pkts_burst[i]->data_len);
  }
#endif
...
int32_t dpdk_recv_pkts_and_gettimeofday(struct mtcp_thread_context *ctxt, int ifidx, struct timeval* cur_ts) {
#ifdef COMPILE_WITH_INTEL_SGX
    int32_t ret = sgx_dpdk_recv_pkts_and_gettimeofday(ctxt, ifidx, cur_ts);

    return ret;
#else
    gettimeofday(cur_ts, NULL);
    return dpdk_recv_pkts(ctxt, ifidx);
#endif
}

int32_t dpdk_send_pkts_and_dpdk_recv_pkts_and_gettimeofday(struct mtcp_thread_context *ctxt, int ifidx, struct timeval* cur_ts) {
#ifdef COMPILE_WITH_INTEL_SGX
    return sgx_dpdk_send_pkts_and_dpdk_recv_pkts_and_gettimeofday(ctxt, ifidx, cur_ts);
#else
    gettimeofday(cur_ts, NULL);
    dpdk_send_pkts(ctxt, ifidx);
    return dpdk_recv_pkts(ctxt, ifidx);
#endif
}

int32_t dpdk_send_merged_pkts_and_dpdk_recv_pkts_and_gettimeofday(struct mtcp_thread_context *ctxt, int ifidx, struct timeval* cur_ts) {
#ifdef COMPILE_WITH_INTEL_SGX
    return sgx_dpdk_send_merged_pkts_and_dpdk_recv_pkts_and_gettimeofday(ctxt, ifidx, cur_ts);
#else
    gettimeofday(cur_ts, NULL);
    dpdk_send_merged_pkts(ctxt, ifidx);
    return dpdk_recv_pkts(ctxt, ifidx);
#endif
}
/*----------------------------------------------------------------------------*/
...
dpdk_destroy_handle(struct mtcp_thread_context *ctxt)
{
	...
	/* free mbufs */
  for (int i = 0; i < num_devices_attached; i++)
...
#define MAX_CHECK_TIME 			90 /* 9s (90 * 100ms) in total */
	...
	for (count = 0; count <= MAX_CHECK_TIME; count++) {
		...
		for (portid = 0; portid < port_num; portid++) {
			...
			if (print_flag == 1) {
				if (link.link_status)
					my_printf("Port %d Link Up - speed %u "
						"Mbps - %s\n", (uint8_t)portid,
...
#define MAX_ROUTE_ENTRY 	64 // from config.c
#define MAX_ARPENTRY        1024 // from arp.h

struct mtcp_config clone_config_outside_enclave(void) {
    struct mtcp_config copyconfig = CONFIG;

    if (CONFIG.eths != NULL) {
        copyconfig.eths = (struct eth_table *) untrusted_calloc(MAX_DEVICES, sizeof(*copyconfig.eths));
        memcpy(copyconfig.eths, CONFIG.eths, MAX_DEVICES * sizeof(*copyconfig.eths));
    }

    if (CONFIG.nif_to_eidx != NULL) {
        copyconfig.nif_to_eidx = (int*)untrusted_calloc(MAX_DEVICES, sizeof(*copyconfig.nif_to_eidx));
        memcpy(copyconfig.nif_to_eidx, CONFIG.nif_to_eidx, MAX_DEVICES * sizeof(*copyconfig.nif_to_eidx));
    }

    if (CONFIG.rtable != NULL) {
        copyconfig.rtable = (struct route_table *)untrusted_calloc(MAX_ROUTE_ENTRY, sizeof(*copyconfig.rtable));
        memcpy(copyconfig.rtable, CONFIG.rtable, MAX_ROUTE_ENTRY * sizeof(*copyconfig.rtable));
    }

    if (CONFIG.gateway != NULL) {
        copyconfig.gateway = (struct route_table *)untrusted_calloc(MAX_ROUTE_ENTRY, sizeof(*copyconfig.gateway));
        memcpy(copyconfig.gateway, CONFIG.gateway, MAX_ROUTE_ENTRY * sizeof(*copyconfig.gateway));
    }

    if (CONFIG.arp.entry != NULL) {
        copyconfig.arp.entry = (struct arp_entry *)untrusted_calloc(MAX_ARPENTRY, sizeof(*copyconfig.arp.entry));
        memcpy(copyconfig.arp.entry, CONFIG.arp.entry, MAX_ARPENTRY * sizeof(*copyconfig.arp.entry));
    }
...
{
#ifdef COMPILE_WITH_INTEL_SGX
    struct mtcp_config copyconfig = clone_config_outside_enclave();
    sgx_dpdk_load_module(copyconfig, num_devices_attached, devices_attached);
#else
	int portid, rxlcore_id, ret;
	/* for Ethernet flow control settings */
	struct rte_eth_fc_conf fc_conf;
	/* setting the rss key */
  // RSS key len must be 40 Bytes long in the case of mellanox NIC
	static uint8_t key[] = {
		...
		/* 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, /1* 50 *1/ */
		/* 0x05, 0x05  /1* 60 - 8 *1/ */
	};
...
#endif
			/* init port */
			my_printf("Initializing port %u... ", (unsigned) portid);
			fflush(stdout);
...
#ifdef DEBUG
			my_printf("Port %u, MAC address: %02X:%02X:%02X:%02X:%02X:%02X\n\n",
			       (unsigned) portid,
...
/*----------------------------------------------------------------------------*/
#ifndef COMPILE_WITH_INTEL_SGX
void dpdk_module_get_dev_info(int nif, char** driver_name, uint64_t* tx_offload_capa, uint64_t* rx_offload_capa) {
    *driver_name = (char*)dev_info[nif].driver_name;
    *tx_offload_capa = dev_info[nif].tx_offload_capa;
    *rx_offload_capa = dev_info[nif].rx_offload_capa;
}
#endif
int32_t
...
#ifdef COMPILE_WITH_INTEL_SGX
	if (dev_info_initialized[nif] == 0) {
	    sgx_get_dev_info(nif, (char**)&(dev_info[nif].driver_name), &dev_info[nif].tx_offload_capa, &dev_info[nif].rx_offload_capa);
        dev_info_initialized[nif] = 1;
	}
#endif
...
io_module_func dpdk_module_func = {
	...
	.recv_pkts_and_gettimeofday		   = dpdk_recv_pkts_and_gettimeofday,
	.send_pkts_and_recv_pkts_and_gettimeofday		   = dpdk_send_pkts_and_dpdk_recv_pkts_and_gettimeofday,
	.prepare_send_merged_pkts		   = dpdk_prepare_send_merged_pkts,
	.send_merged_pkts_and_recv_pkts_and_gettimeofday		   = dpdk_send_merged_pkts_and_dpdk_recv_pkts_and_gettimeofday
};
/*----------------------------------------------------------------------------*/
#else
io_module_func dpdk_module_func = {
